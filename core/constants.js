...
/**
 * Erlaubte Ziele der generischen `call`-Aktion (data-action mit Wert 'call', Zielname im
 * data-value-Attribut). SEC-03: explizite Allowlist statt Namenspräfix-Konvention — ein
 * nicht gelisteter Name führt zu keinem Aufruf von window[name]. Wird aus den entsprechenden
 * Attributpaaren im Quellbaum abgeleitet (siehe tests/unit/event-delegation.test.js,
 * Abgleichs-Testfall); ein neuer `call`-Knopf ohne Eintrag hier funktioniert bewusst nicht.
 * @type {Set<string>}
 */
const CALL_ACTION_WHITELIST = new Set([
    ...
    'assignItems',
    'assignSpells',
    'assignSpellsSelectAll',
    'assignSpellsSelectNone',
    'cancelCharEdit',
    'cancelEncEdit',
    'cancelLinkEdit',
    'cancelPresetEdit',
