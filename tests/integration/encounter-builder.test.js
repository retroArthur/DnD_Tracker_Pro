...
describe('Encounter Builder Integration', () => {
    ...
    const getMultiplier = (monsterCount, partySize) => {
        const multipliers = [
            { count: 1, mult: 1 }, { count: 2, mult: 1.5 },
            { count: 3, mult: 2 }, { count: 7, mult: 2.5 },
            { count: 11, mult: 3 }, { count: 15, mult: 4 }
        ];
    ...
    describe('Party-Analyse', () => {
        ...
        test('sollte gemischte Party-Level verarbeiten', () => {
            ...
            const avgLevel = dataStore.characters.reduce((sum, c) => sum + c.level, 0)
    ...
    describe('XP-Belohnungen', () => {
        ...
        test('sollte Gesamt-XP für Session berechnen', () => {
            ...
            const sessionXP = encounters.reduce((total, enc) =>
                total + Math.round(enc.baseXP * enc.multiplier), 0);
