...
// Rest Modal anzeigen
export function showRestModal(): void {
    ...
    if (!modal) {
        ...
        modal.onclick = (e: MouseEvent) => { if (e.target === modal) hideModal('rest-modal'); };
...
export function selectRestType(type: string): void {
    const D = (window as any).D;
...
function renderRestDetails(type: string): string {
    if (type === 'short') {
        return `
            <div class="rest-info">
...
function renderRestCharacters(characters: any[], type: string): string {
    if (!characters.length) {
...
function getHitDieType(className: string): number {
    const hitDice: Record<string, number> = {
        'Barbar': 12,
        'Kämpfer': 10, 'Paladin': 10, 'Waldläufer': 10,
        'Barde': 8, 'Druide': 8, 'Kleriker': 8, 'Mönch': 8, 'Schurke': 8, 'Hexenmeister': 8,
        'Magier': 6, 'Zauberer': 6,
        'Artificer': 8, 'Blutjäger': 10
    };
    return hitDice[className] || 8;
}

export function adjustRestHitDice(charId: number | string, delta: number): void {
    const id = typeof charId === 'string' ? parseInt(charId) : charId;
    const input = $(`rest-hd-${id}`) as HTMLInputElement | null;
    if (!input) return;
...
export function applyRest(): void {
    const D = (window as any).D;
    ...
    D.characters.forEach((char: any) => {
        ...
        if (isLongRest) {
            ...
            const hdInput = $(`rest-hd-${char.id}`) as HTMLInputElement | null;
...
export function quickShortRest(charId: number | string): void {
...
(window as any).showRestModal = showRestModal;
(window as any).selectRestType = selectRestType;
(window as any).adjustRestHitDice = adjustRestHitDice;
(window as any).applyRest = applyRest;
(window as any).quickShortRest = quickShortRest;
