...
export function showLootDistributionModal(): void {
    ...
    if (!modal) {
        ...
        modal.onclick = (e: MouseEvent) => { if (e.target === modal) hideModal('loot-dist-modal'); };
...
function calculatePartyGold(): number {
    const D = (window as any).D;
...
function renderGoldSplit(amount: number, characters: any[]): string {
...
function renderDistributionCharacters(characters: any[]): string {
...
export function updateGoldSplit(): void {
    const amountEl = $('ld-gold-amount') as HTMLInputElement | null;
...
function getIncludedCharacters(): any[] {
    const included: any[] = [];
...
export function applyGoldSplit(): void {
    const D = (window as any).D;
...
export function collectAllGold(): void {
    const D = (window as any).D;
    ...
    // Update modal
    const amountEl = $('ld-gold-amount') as HTMLInputElement;
...
(window as any).showLootDistributionModal = showLootDistributionModal;
(window as any).updateGoldSplit = updateGoldSplit;
(window as any).applyGoldSplit = applyGoldSplit;
(window as any).collectAllGold = collectAllGold;
