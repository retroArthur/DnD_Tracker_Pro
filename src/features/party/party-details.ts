 ...
 */
export function showCharacterDetails(id: number | string): void {
    ...
    const attrMod = (val: number): string => {
            ...
            <div class="char-inventory-row">
                <div class="char-inventory-box">
                    ...
                    <div class="char-inventory-content">
                        ${spells.length ? spells.map((s: any) => `<span class="char-tag spell clickable" data-action="navigate-entity-stop" data-type="spells" data-id="${s.id}" title="Klicken für Details">${esc(s.name)}</span>`).join('') : '<span class="char-empty">Keine Zauber</span>'}
                    </div>
                </div>
                <div class="char-inventory-box">
                    <div class="char-inventory-header" data-action="toggle-parent-expanded">
                    ...
                    <div class="char-inventory-content">
                        ${items.length ? items.map((i: any) => `<span class="char-tag item clickable" data-action="navigate-entity-stop" data-type="loot" data-id="${i.id}" title="Klicken für Details">${CATS[i.category]?.split(' ')[0] || '📦'} ${esc(i.name)}${i.assignedQty > 1 ? ` ×${i.assignedQty}` : ''}</span>`).join('') : '<span class="char-empty">Keine Items</span>'}
                    </div>
                </div>
            </div>
