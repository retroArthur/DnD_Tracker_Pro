...
import { EntityLookup, renderEmptyState, updateCounters, populateFilterDropdown } from '@render/helpers';
...
export function renderParty(): void {
    ...
    const renderConditionsBar = (window as any).renderConditionsBar;
    const CATS = (window as any).CATS;
    const repairCharactersData = (window as any).repairCharactersData;
    const updateDiceCharSelect = (window as any).updateDiceCharSelect;
    ...
    const searchInput = $('party-search') as HTMLInputElement | null;
    const search = (searchInput?.value || '').toLowerCase();
    const classFilterEl = $('party-class-filter') as HTMLSelectElement | null;
    const classFilter = classFilterEl?.value || '';

    // Klassen-Filter-Dropdown befüllen
    const classes = [...new Set(D.characters.map((ch: any) => ch.characterClass).filter(Boolean))].sort();
    ...
    if (classFilter) {
        characters = characters.filter((ch: any) => ch.characterClass === classFilter);
    }

    // Suche anwenden
    if (search) {
        characters = characters.filter((ch: any) =>
    ...
    c.innerHTML = characters.map((ch: any) => renderCharacterCard(ch, renderConditionsBar, CATS)).join('');
 ...
 */
export function renderPartyRoster(container: HTMLElement, characters: any[]): void {
    if (!characters.length) {
        container.innerHTML = `
 ...
 */
export function renderCharacterCard(ch: any, renderConditionsBar: any, CATS: any): string {
    const hpPct = ch.hpMax > 0 ? (ch.hpCurrent / ch.hpMax) * 100 : 100;
    const hpClass = hpPct <= 25 ? 'critical' : hpPct <= 50 ? 'bloodied' : 'healthy';
    ...
    // Spells & Items
    const spells = (ch.spells || []).map((sid: number) => EntityLookup.spell(sid)).filter(Boolean);
 ...
 */
export function renderCompactSpellSlots(ch: any): string {
 ...
 */
export function scrollToChar(id: number | string): void {
 ...
 */
export function renderPartyOverview(): void {
    const D = (window as any).D;
    ...
    // Calculate stats
    const passivePerceptions = chars.map((c: any) => c.passivePerception || 10);
    const lowestPerception = Math.min(...passivePerceptions);

    const acValues = chars.map((c: any) => c.armorClass || 10);
    const minAC = Math.min(...acValues);
    const maxAC = Math.max(...acValues);
    const acRange = minAC === maxAC ? `${minAC}` : `${minAC}-${maxAC}`;

    const totalHP = chars.reduce((sum: number, c: any) => sum + (c.hpMax || 0), 0);
    const currentHP = chars.reduce((sum: number, c: any) => sum + (c.hpCurrent || 0), 0);
    ...
    // Count conditions
    const totalConditions = chars.reduce((sum: number, c: any) => sum + (c.conditions?.length || 0), 0);
...
(window as any).renderParty = renderParty;
(window as any).renderPartyRoster = renderPartyRoster;
(window as any).renderCharacterCard = renderCharacterCard;
(window as any).renderCompactSpellSlots = renderCompactSpellSlots;
(window as any).scrollToChar = scrollToChar;
(window as any).renderPartyOverview = renderPartyOverview;
