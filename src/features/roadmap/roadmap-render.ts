...
export function renderRoadmapConnections(): void {
    ...
    if (!D.roadmap?.connections) return;

    D.roadmap.connections.forEach((conn: any) => {
        const fromEvent = D.roadmap.events.find((e: any) => e.id === conn.from);
        const toEvent = D.roadmap.events.find((e: any) => e.id === conn.to);
...
function updateSVGViewBox(): void {
    ...
    if (log) log('[ROADMAP] Viewport gesetzt auf fixe Größe:', FIXED_WIDTH, 'x', FIXED_HEIGHT);
}

function renderConnection(svg: Element, fromEvent: any, toEvent: any, conn: any): void {
    const deleteRoadmapConnection = (window as any).deleteRoadmapConnection;
    ...
    const fromTile = document.querySelector(`.roadmap-event[data-id="${fromEvent.id}"]`) as HTMLElement | null;
    const toTile = document.querySelector(`.roadmap-event[data-id="${toEvent.id}"]`) as HTMLElement | null;
    ...
    if (style === '7') {
        ...
        (outerPath as any).dataset.id = conn.id + '-outer';
    ...
    (path as any).dataset.id = conn.id;
...
function getPinCoordinates(event: any, pinPosition: string, tileWidth: number, tileHeight: number): { x: number; y: number } {
    const baseX = event.x;
    const baseY = event.y;
...
export function setupRoadmapSVGMarkers(): void {
    const svg = $('roadmap-svg');
    if (!svg) return;
    ...
    markerTypes.forEach(({ type, color }) => {
        ...
        if (defs!.querySelector(`#roadmap-arrow-${type}`)) return;
...
export function renderRoadmapEvents(): void {
    const eventsContainer = $('roadmap-events');
...
function cleanupEventTiles(): void {
    const eventsContainer = $('roadmap-events');
    if (!eventsContainer) return;
    ...
    tiles.forEach(tile => {
        const element = tile as any;
        if (element._cleanupHandlers) {
            element._cleanupHandlers.forEach((cleanup: () => void) => cleanup());
            element._cleanupHandlers = null;
        }
    });
}

function createEventTile(event: any): HTMLDivElement {
    const getEventTypeIcon = (window as any).getEventTypeIcon;
    const startEventDrag = (window as any).startEventDrag;
    const handlePinClickForConnection = (window as any).handlePinClickForConnection;
    const startConnectionMode = (window as any).startConnectionMode;
    const roadmapConnectionMode = (window as any).roadmapConnectionMode;
    ...
    if (infoBtn && infoPopup) {
        ...
        (div as any)._cleanupHandlers.push(() => {
    ...
    pins.forEach(pin => {
        ...
        pin.addEventListener('dblclick', (e: Event) => {
            e.stopPropagation();
            const pinElement = pin as HTMLElement;
            const pinPosition = pinElement.dataset.pin;
...
function renderLinkedEntities(event: any): string {
    const D = (window as any).D;
    const links: string[] = [];

    // NPCs
    if (event.linkedNPCs?.length > 0) {
        event.linkedNPCs.forEach((npcId: number) => {
            const npc = D.npcs?.find((n: any) => n.id === npcId);
            if (npc) {
    ...
    if (event.linkedQuests?.length > 0) {
        event.linkedQuests.forEach((questId: number) => {
            const quest = D.quests?.find((q: any) => q.id === questId);
            if (quest) {
    ...
    if (event.linkedLocations?.length > 0) {
        event.linkedLocations.forEach((locId: number) => {
            const loc = D.locations?.find((l: any) => l.id === locId);
            if (loc) {
    ...
    if (event.linkedEncounters?.length > 0) {
        event.linkedEncounters.forEach((encId: number) => {
            const enc = D.encounters?.find((e: any) => e.id === encId);
            if (enc) {
...
export function renderRoadmapToolbar(): void {
    const D = (window as any).D;
...
export function renderRoadmapControls(): void {
...
export function initRoadmapRendering(): void {
...
(window as any).renderRoadmapConnections = renderRoadmapConnections;
(window as any).renderRoadmapEvents = renderRoadmapEvents;
(window as any).renderRoadmapToolbar = renderRoadmapToolbar;
(window as any).renderRoadmapControls = renderRoadmapControls;
(window as any).setupRoadmapSVGMarkers = setupRoadmapSVGMarkers;
(window as any).initRoadmapRendering = initRoadmapRendering;
