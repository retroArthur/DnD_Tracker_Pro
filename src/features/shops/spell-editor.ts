...
let hideFloatingToolbarTimeout: number | null = null;
let floatingToolbarInteracting: boolean = false;
let floatingToolbarInitialized: boolean = false;
let currentContextTable: HTMLTableElement | null = null;
let currentContextLink: HTMLAnchorElement | null = null;
let contextToolbarsInitialized: boolean = false;
let editorHandlersInitialized: boolean = false;
...
export function renderSpells(): void {
    ...
    const currentSpellFilter = (window as any).currentSpellFilter || 'all';
    const currentSpellLevelFilter = (window as any).currentSpellLevelFilter || 'all';
    const currentSpellSchoolFilter = (window as any).currentSpellSchoolFilter || 'all';
    const SPELLS_PER_PAGE = (window as any).SPELLS_PER_PAGE || 30;
    ...
    const searchInput = $('spell-search') as HTMLInputElement | null;
    const classFilterInput = $('spell-class-filter') as HTMLSelectElement | null;
    ...
    spells = spells.filter((s: any) => {
        ...
        if (currentSpellSchoolFilter !== 'all' && s.school !== currentSpellSchoolFilter) return false;
        ...
        if (search) {
            ...
            if (!name.includes(search) && !school.includes(search) &&
                ...
                !note.includes(search)) return false;
        }
...
function renderSpellCards(spells: any[]): string {
    return spells.map(s => {
...
function renderLoadMoreButton(total: number, perPage: number): string {
...
export function loadMoreSpells(): void {
    const SPELLS_PER_PAGE = (window as any).SPELLS_PER_PAGE || 30;
    const c = $('spell-list');
...
export function toggleSpellCard(id: number): void {
...
export function expandAllSpells(): void {
    const D = (window as any).D;
    (D.spells || []).forEach((s: any) => expandedSpells.add(s.id));
    renderSpells();
}

export function collapseAllSpells(): void {
    expandedSpells.clear();
    renderSpells();
}

export function setSpellFilter(f: string): void {
    (window as any).currentSpellFilter = f;
    renderSpells();
}

export function setSpellLevelFilter(f: string): void {
    (window as any).currentSpellLevelFilter = f;
    renderSpells();
}

export function setSpellSchoolFilter(f: string): void {
    (window as any).currentSpellSchoolFilter = f;
    renderSpells();
}
...
export function onSpellRangeChange(): void {
    const sel = $('spell-range-select') as HTMLSelectElement | null;
    const custom = $('spell-range-custom') as HTMLInputElement | null;
...
export function onSpellTimeChange(): void {
    const sel = $('spell-time-select') as HTMLSelectElement | null;
    const custom = $('spell-time-custom') as HTMLInputElement | null;
    if (!sel || !custom) return;
...
export function onSpellDurationChange(): void {
    const sel = $('spell-duration-select') as HTMLSelectElement | null;
    const custom = $('spell-duration-custom') as HTMLInputElement | null;
    if (!sel || !custom) return;
...
export function toggleMaterialField(): void {
    const mChecked = ($('spell-m') as HTMLInputElement | null)?.checked || false;
...
export function formatText(elementId: string, format: string, value?: string): void {
...
export function setEditorFont(elementIdOrSelect: string | HTMLSelectElement, selectEl?: HTMLSelectElement): void {
    const EDITOR_FONTS = (window as any).EDITOR_FONTS;
    let editorId: string;
    ...
    if (typeof elementIdOrSelect === 'object' && elementIdOrSelect.tagName === 'SELECT') {
        ...
        editorId = elementIdOrSelect as string;
        select = selectEl!;
    }
...
export function setEditorFontSize(elementIdOrSelect: string | HTMLSelectElement, selectEl?: HTMLSelectElement): void {
    let editorId: string;
    let select: HTMLSelectElement;
    ...
    fontElements.forEach(el => {
        ...
        (el as HTMLElement).style.fontSize = select.value;
    });
}

export function clearEditorFormatting(elementId: string): void {
    const editor = $(elementId);
    if (!editor) return;
...
export function setBorderFormat(elementId: string): void {
    const editor = $(elementId);
    if (!editor) return;
    editor.focus();
...
export function setReadAloudFormat(elementId: string, style: string = 'parchment'): void {
    const editor = $(elementId);
    if (!editor) return;
    editor.focus();
    ...
    if (selection && selection.rangeCount > 0) {
        ...
        const existingBlock = range.commonAncestorContainer.nodeType === Node.TEXT_NODE
            ? (range.commonAncestorContainer as Text).parentElement?.closest('.read-aloud')
            : (range.commonAncestorContainer as HTMLElement).closest?.('.read-aloud');
        ...
        if (existingBlock) {
            ...
            range.insertNode(wrapper);
            const styleNames: Record<string, string> = { parchment: 'Pergament', crimson: 'Karmesin', violet: 'Violett', sage: 'Salbei', sky: 'Himmel', slate: 'Schiefer' };
...
function removeSelectionBorders(): void {
    ...
    const editor = container.nodeType === Node.TEXT_NODE
        ? (container as Text).parentElement?.closest('.rich-editor, .spell-editor, .dialog-text')
        : (container as HTMLElement).closest?.('.rich-editor, .spell-editor, .dialog-text');
...
export function initEditorPasteHandlers(): void {
...
function handleEditorKeydown(e: KeyboardEvent): void {
    if (e.key === 'T' && e.ctrlKey && e.shiftKey) {
        e.preventDefault();
        floatingToolbarTarget = e.target as HTMLElement;
        insertTable();
        return;
    }
...
export function insertTable(rows: number = 3, cols: number = 3): void {
    const editor = floatingToolbarTarget || document.activeElement as HTMLElement;
...
export function updateStickyOffsets(): void {
    ...
    if (isMobile) {
        ...
        const encounterControls = document.querySelector('.encounter-controls') as HTMLElement | null;
...
export function initFloatingToolbar(): void {
    ...
    const EDITOR_FONTS = (window as any).EDITOR_FONTS;
    const TOOLBAR_DIMENSIONS = (window as any).TOOLBAR_DIMENSIONS;
    const toolbar = $('floating-toolbar');
    if (!toolbar) return;

    document.querySelectorAll('.editor-toolbar').forEach(editorToolbar => {
        editorToolbar.addEventListener('mousedown', (e: Event) => {
            const target = e.target as HTMLElement;
    ...
    toolbar.addEventListener('click', (e: Event) => {
        const target = e.target as HTMLElement;
        const btn = target.closest('[data-floating-action]') as HTMLElement | null;
        const colorSwatch = target.closest('.color-swatch') as HTMLElement | null;
    ...
    toolbar.addEventListener('change', (e: Event) => {
        const target = e.target as HTMLElement;
        const select = target.closest('[data-floating-action]') as HTMLSelectElement | null;
    ...
    function applyFloatingFormat(action: string, editor: HTMLElement, savedRange: Range): void {
        ...
        if (tagMap[action]) {
            ...
            const parentTag = (range.commonAncestorContainer as HTMLElement).parentElement?.closest(tag);
            ...
            const parentList = (range.commonAncestorContainer as HTMLElement).parentElement?.closest('ul, ol');
...
function handleSelectionChange(): void {
    ...
    const editor = anchorNode.nodeType === Node.TEXT_NODE
        ? (anchorNode as Text).parentElement?.closest('.rich-editor, .spell-editor, .dialog-text, .cf-notes-editor')
        : (anchorNode as HTMLElement).closest?.('.rich-editor, .spell-editor, .dialog-text, .cf-notes-editor');
    ...
    floatingToolbarTarget = editor as HTMLElement;
...
export function hideFloatingToolbar(clearRange: boolean = true): void {
    const toolbar = $('floating-toolbar');
    if (toolbar) {
...
function applyFloatingHighlight(color: string, editor: HTMLElement, savedRange: Range): void {
    let selection = window.getSelection();
...
export function initContextToolbars(): void {
    ...
    tableToolbar.addEventListener('click', (e: Event) => {
        const target = e.target as HTMLElement;
        const btn = target.closest('[data-table-action]') as HTMLElement | null;
        ...
        const cell = selection?.anchorNode?.nodeType === Node.TEXT_NODE
            ? (selection.anchorNode as Text).parentElement?.closest('td, th')
            : (selection?.anchorNode as HTMLElement)?.closest?.('td, th');

        const row = cell?.parentElement as HTMLTableRowElement | null;
        const rowIndex = row ? Array.from(table.rows).indexOf(row) : -1;
        const cellIndex = cell && row ? Array.from(row.cells).indexOf(cell as HTMLTableCellElement) : -1;
    ...
    linkToolbar.addEventListener('click', (e: Event) => {
        const target = e.target as HTMLElement;
        const btn = target.closest('[data-link-action]') as HTMLElement | null;
    ...
    document.addEventListener('click', (e: Event) => {
        ...
        if (table && table.closest(editorSelector)) {
            currentContextTable = table as HTMLTableElement;
            showTableContextToolbar(table as HTMLTableElement);
        } else {
        ...
        if (link && link.closest(editorSelector)) {
            currentContextLink = link as HTMLAnchorElement;
            showLinkContextToolbar(link as HTMLAnchorElement);
        } else {
...
function showTableContextToolbar(table: HTMLTableElement): void {
...
function hideTableContextToolbar(): void {
    const toolbar = $('table-context-toolbar');
    if (toolbar) toolbar.classList.remove('visible');
    currentContextTable = null;
}

function showLinkContextToolbar(link: HTMLAnchorElement): void {
...
function hideLinkContextToolbar(): void {
    const toolbar = $('link-context-toolbar');
    if (toolbar) toolbar.classList.remove('visible');
    currentContextLink = null;
}

export function hideContextToolbars(): void {
    hideTableContextToolbar();
    hideLinkContextToolbar();
}
...
function getSpellClassesFromCheckboxes(): string[] {
    ...
    classIds.forEach(id => {
        const cb = $('spell-class-' + id) as HTMLInputElement | null;
        if (cb?.checked) classes.push(cb.value);
    });
    return classes;
}

function setSpellClassesCheckboxes(classes: string[]): void {
...
export function saveSpell(): void {
    const D = (window as any).D;
    const nextId = (window as any).nextId;

    const editIdInput = $('edit-spell-id') as HTMLInputElement | null;
    const nameInput = $('spell-name') as HTMLInputElement | null;
    const typeInput = $('spell-type') as HTMLSelectElement | null;
    const levelInput = $('spell-level') as HTMLInputElement | null;
    const schoolInput = $('spell-school') as HTMLSelectElement | null;
    const rangeSelectInput = $('spell-range-select') as HTMLSelectElement | null;
    const rangeCustomInput = $('spell-range-custom') as HTMLInputElement | null;
    const timeSelectInput = $('spell-time-select') as HTMLSelectElement | null;
    const timeCustomInput = $('spell-time-custom') as HTMLInputElement | null;
    const durationSelectInput = $('spell-duration-select') as HTMLSelectElement | null;
    const durationCustomInput = $('spell-duration-custom') as HTMLInputElement | null;
    const ritualInput = $('spell-ritual') as HTMLInputElement | null;
    const vInput = $('spell-v') as HTMLInputElement | null;
    const gInput = $('spell-g') as HTMLInputElement | null;
    const mInput = $('spell-m') as HTMLInputElement | null;
    const materialInput = $('spell-material') as HTMLInputElement | null;
    ...
    if (id) {
        const idx = D.spells.findIndex((x: any) => x.id === parseEntityId(id));
...
export function editSpell(id: number): void {
    ...
    const timeSelect = $('spell-time-select') as HTMLSelectElement | null;
    const timeCustom = $('spell-time-custom') as HTMLInputElement | null;
    ...
    const rangeSelect = $('spell-range-select') as HTMLSelectElement | null;
    const rangeCustom = $('spell-range-custom') as HTMLInputElement | null;
    const range = s.range || '';
    ...
    const durationSelect = $('spell-duration-select') as HTMLSelectElement | null;
    const durationCustom = $('spell-duration-custom') as HTMLInputElement | null;
...
export function deleteSpell(id: number): void {
    const D = (window as any).D;
    const spell = EntityLookup.spell(id);
    if (confirm(`Zauber "${spell?.name || 'Unbekannt'}" löschen?`)) {
        pushUndo('Zauber gelöscht');
        D.spells = D.spells.filter((s: any) => s.id !== id);
        renderSpells();
        save();
    }
}

function clearSpellForm(): void {
    const editIdInput = $('edit-spell-id') as HTMLInputElement | null;
    const nameInput = $('spell-name') as HTMLInputElement | null;
    const typeInput = $('spell-type') as HTMLSelectElement | null;
    const levelInput = $('spell-level') as HTMLInputElement | null;
...
(window as any).debouncedRenderSpells = debouncedRenderSpells;
(window as any).renderSpells = renderSpells;
(window as any).loadMoreSpells = loadMoreSpells;
(window as any).toggleSpellCard = toggleSpellCard;
(window as any).expandAllSpells = expandAllSpells;
(window as any).collapseAllSpells = collapseAllSpells;
(window as any).setSpellFilter = setSpellFilter;
(window as any).setSpellLevelFilter = setSpellLevelFilter;
(window as any).setSpellSchoolFilter = setSpellSchoolFilter;
(window as any).onSpellRangeChange = onSpellRangeChange;
(window as any).onSpellTimeChange = onSpellTimeChange;
(window as any).onSpellDurationChange = onSpellDurationChange;
(window as any).toggleMaterialField = toggleMaterialField;
(window as any).formatText = formatText;
(window as any).setEditorFont = setEditorFont;
(window as any).setEditorFontSize = setEditorFontSize;
(window as any).clearEditorFormatting = clearEditorFormatting;
(window as any).setBorderFormat = setBorderFormat;
(window as any).setReadAloudFormat = setReadAloudFormat;
(window as any).initEditorPasteHandlers = initEditorPasteHandlers;
(window as any).insertTable = insertTable;
(window as any).updateStickyOffsets = updateStickyOffsets;
(window as any).initFloatingToolbar = initFloatingToolbar;
(window as any).hideFloatingToolbar = hideFloatingToolbar;
(window as any).initContextToolbars = initContextToolbars;
(window as any).hideContextToolbars = hideContextToolbars;
(window as any).saveSpell = saveSpell;
(window as any).editSpell = editSpell;
(window as any).deleteSpell = deleteSpell;
