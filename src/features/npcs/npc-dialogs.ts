 ...
 */
export function addDialogField(): void {
    const handleEditorPaste = (window as any).handleEditorPaste;
    ...
    if (editor) editor.addEventListener('paste', handleEditorPaste as EventListener);
}

/**
 * Shows modal to add a quick dialog to an NPC
 */
export function showAddDialogModal(npcId: number | string): void {
 ...
 */
export function saveQuickDialog(npcId: number | string): void {
    const renderNPCList = (window as any).renderNPCList;
    const npc = EntityLookup.npc(npcId);
    if (!npc) return;

    const textEl = $('quick-dialog-text') as HTMLElement | null;
    const titleInput = $('quick-dialog-title') as HTMLInputElement | null;
    const triggerInput = $('quick-dialog-trigger') as HTMLInputElement | null;
 ...
 */
export function resetDialogFieldCounter(): void {
...
(window as any).addDialogField = addDialogField;
(window as any).showAddDialogModal = showAddDialogModal;
(window as any).saveQuickDialog = saveQuickDialog;
(window as any).resetDialogFieldCounter = resetDialogFieldCounter;
