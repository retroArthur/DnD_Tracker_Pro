...
/**
 * Shows NPC popup on hover/click
 */
export function showNPCPopup(npcId: number | string, event: MouseEvent): void {
    ...
    const tags: string[] = [];
    if (npc.quests?.length) tags.push(...npc.quests.map((q: string) => `<span class="chip color-green" style="font-size:0.75em;">📜 ${esc(q)}</span>`));
    if (npc.info?.length) tags.push(...npc.info.map((i: string) => `<span class="chip color-purple" style="font-size:0.75em;">💡 ${esc(i)}</span>`));
    if (npc.relationships?.length) tags.push(...npc.relationships.map((r: string) => `<span class="chip color-pink" style="font-size:0.75em;">🔗 ${esc(r)}</span>`));

    // Trigger
    const triggers = (npc.triggers || []).map((t: any) => `
    ...
    // Dialoge (erste 2)
    const dialogs = (npc.dialogs || []).slice(0, 2).map((d: any) => `
 ...
 * Closes NPC popup
 */
export function closeNPCPopup(): void {
 ...
 */
function closeNPCPopupOnOutsideClick(e: MouseEvent): void {
    const popup = $('npc-popup');
...
(window as any).showNPCPopup = showNPCPopup;
(window as any).closeNPCPopup = closeNPCPopup;
(window as any).closeNPCPopupOnOutsideClick = closeNPCPopupOnOutsideClick;
