...
const UI_TIMING = (window as any).UI_TIMING;
 ...
 */
const DEFAULT_DMSCREEN_LAYOUT: DMScreenLayout = {
    widgets: [
 ...
 */
const DEFAULT_DMSCREEN_PROFILES: { [key: string]: DMScreenProfile } = {
    'standard': {
        name: 'Standard',
        icon: '📋',
        widgets: [
 ...
 */
let dmsLiveSyncTimer: number | null = null;
const DMS_LIVE_SYNC_DELAY: number = UI_TIMING.DM_SCREEN_SYNC_DELAY;

/**
 * Prüft ob DM Screen aktuell sichtbar ist
 */
function isDMScreenVisible(): boolean {
 ...
 */
function refreshDMScreenIfVisible(): void {
 ...
 */
function renderDMScreenWidgetsOnly(): void {
    if (!isDMScreenVisible()) return;
    ...
    allWidgets.filter((w: DMScreenWidget) => w.visible).forEach((widget: DMScreenWidget) => {
 ...
 */
function setupDMScreenLiveSync(): void {
    // Überschreibe die globale save-Funktion um Live-Sync zu triggern
    if (typeof (window as any)._originalSave === 'undefined' && typeof save === 'function') {
        (window as any)._originalSave = save;
        (window as any).save = function() {
            (window as any)._originalSave.apply(this, arguments);
 ...
 */
function initDMScreenLayout(): void {
    if (!D.dmScreenLayout) {
 ...
 */
export function resetDMScreenLayout(): void {
 ...
 */
export function switchDMSProfile(profileId: string): void {
 ...
 */
export function saveDMSProfileAs(): void {
 ...
 */
export function deleteDMSProfile(profileId: string): void {
 ...
 */
export function toggleDMSProfileDropdown(): void {
 ...
 */
function renderDMSProfileList(): void {
    ...
    if (customProfiles.length > 0) {
        ...
        for (const [id, profile] of customProfiles) {
            ...
            html += `
                <div class="dms-profile-item ${isActive ? 'active' : ''}" data-action="dms-switch-profile" data-profile="${id}">
                    <span class="dms-profile-icon">${(profile as DMScreenProfile).icon}</span>
                    <span class="dms-profile-name">${esc((profile as DMScreenProfile).name)}</span>
                    ${isActive ? '<span class="dms-profile-check">✓</span>' : ''}
 ...
 */
export function renderDMScreen(): void {
    initDMScreenLayout();
    ...
    const allWidgets = D.dmScreenLayout.widgets.filter((w: DMScreenWidget) => w.visible);
    const widgetDefs = getDMScreenWidgets();

    // Separate compact (Quick Bar) and regular (Grid) widgets
    const compactWidgets = allWidgets.filter((w: DMScreenWidget) => {
        const def = widgetDefs[w.type];
        return def && def.compact === true;
    });
    const gridWidgets = allWidgets.filter((w: DMScreenWidget) => {
        const def = widgetDefs[w.type];
        return def && def.compact !== true;
    });
    ...
    if (quickBar) {
        if (compactWidgets.length > 0) {
            quickBar.innerHTML = compactWidgets.map((widget: DMScreenWidget) => {
                const def = widgetDefs[widget.type];
                try {
                    return def.render();
                } catch (err) {
            ...
            (quickBar as HTMLElement).style.display = 'flex';
        } else {
            quickBar.innerHTML = '';
            (quickBar as HTMLElement).style.display = 'none';
        }
    }
    ...
    try {
        grid.innerHTML = gridWidgets.map((widget: DMScreenWidget) => {
            const def = widgetDefs[widget.type];
            if (!def) {
                if (APP_CONFIG.DEBUG_MODE) {
 ...
 */
function renderDMSConfigList(): void {
    ...
    list.innerHTML = allWidgets.map((widget: DMScreenWidget) => {
        const def = widgetDefs[widget.type];
        if (!def) return '';
 ...
 */
export function toggleDMSWidget(widgetId: string): void {
    const widget = D.dmScreenLayout.widgets.find((w: DMScreenWidget) => w.id === widgetId);
    if (widget) {
 ...
 */
export function hideDMSWidget(widgetId: string): void {
    const widget = D.dmScreenLayout.widgets.find((w: DMScreenWidget) => w.id === widgetId);
    if (widget) {
 ...
 */
function saveDMScreenLayout(): void {
 ...
 */
export function toggleDMSConfigDropdown(): void {
...
let dmsDraggedWidget: HTMLElement | null = null;

function initDMSWidgetDragDrop(): void {
    const grid = $('dmscreen-grid');
    if (!grid) return;
    ...
    widgets.forEach(widget => {
        widget.addEventListener('dragstart', handleDMSWidgetDragStart as EventListener);
        widget.addEventListener('dragend', handleDMSWidgetDragEnd as EventListener);
        widget.addEventListener('dragover', handleDMSWidgetDragOver as EventListener);
        widget.addEventListener('drop', handleDMSWidgetDrop as EventListener);
        widget.addEventListener('dragenter', handleDMSWidgetDragEnter as EventListener);
        widget.addEventListener('dragleave', handleDMSWidgetDragLeave as EventListener);
    });
}

function handleDMSWidgetDragStart(this: HTMLElement, e: DragEvent): void {
...
function handleDMSWidgetDragOver(e: DragEvent): void {
    e.preventDefault();
    if (e.dataTransfer) {
...
function handleDMSWidgetDragEnter(this: HTMLElement, e: DragEvent): void {
    e.preventDefault();
...
function handleDMSWidgetDrop(this: HTMLElement, e: DragEvent): void {
    e.preventDefault();
    this.classList.remove('drag-over');
...
function reorderDMSWidgets(draggedId: string, targetId: string): void {
    const widgets = D.dmScreenLayout.widgets;
    const draggedIdx = widgets.findIndex((w: DMScreenWidget) => w.id === draggedId);
    const targetIdx = widgets.findIndex((w: DMScreenWidget) => w.id === targetId);
...
let dmsConfigDraggedItem: HTMLElement | null = null;

function initDMSConfigDragDrop(): void {
    const list = $('dms-config-list');
    if (!list) return;
    ...
    items.forEach(item => {
        ...
        item.addEventListener('dragstart', handleDMSConfigDragStart as EventListener);
        item.addEventListener('dragend', handleDMSConfigDragEnd as EventListener);
        item.addEventListener('dragover', handleDMSConfigDragOver as EventListener);
        item.addEventListener('drop', handleDMSConfigDrop as EventListener);
        item.addEventListener('dragenter', handleDMSConfigDragEnter as EventListener);
        item.addEventListener('dragleave', handleDMSConfigDragLeave as EventListener);
    });
}

function handleDMSConfigDragStart(this: HTMLElement, e: DragEvent): void {
...
function handleDMSConfigDragOver(e: DragEvent): void {
    e.preventDefault();
    if (e.dataTransfer) {
        e.dataTransfer.dropEffect = 'move';
    }
...
function handleDMSConfigDragEnter(this: HTMLElement, e: DragEvent): void {
    e.preventDefault();
...
function handleDMSConfigDrop(this: HTMLElement, e: DragEvent): void {
    e.preventDefault();
    this.classList.remove('drag-over');
...
function getDMScreenWidgets(): { [key: string]: WidgetDefinition } {
    return {
        'party': {
            name: 'Party Stats',
            icon: '👥',
 ...
 */
function renderDMSConditionsCompact(): string {
    return `
...
function renderDMSPartyWidget(): string {
    const chars: Character[] = D.characters || [];
    if (chars.length === 0) {
...
function renderDMSInitiativeWidget(): string {
    const combatants: Combatant[] = D.initiative?.combatants || [];
    const currentTurn: number = D.initiative?.currentTurn || 0;
    const round: number = D.initiative?.round || 1;
...
function renderDMSDiceWidget(): string {
    return `
        <div class="dms-dice">
 ...
 */
export function dmsRollDice(formula: string): number {
    let result = 0;
    let rolls: number[] = [];

    // Use parseDiceNotation if available (returns object with total, rolls)
    if (typeof (window as any).parseDiceNotation === 'function') {
        const parsed = (window as any).parseDiceNotation(formula);
        if (parsed) {
            result = parsed.total;
            rolls = parsed.rolls;
        }
    ...
    if (typeof (window as any).addToDiceHistory === 'function') {
        (window as any).addToDiceHistory(formula, result, rolls);
    }
...
function renderDMSDCWidget(): string {
    const dcs: DCEntry[] = [
...
function renderDMSTablesWidget(): string {
    const tables: RandomTable[] = D.randomTables || [];
...
function renderDMSRulesWidget(): string {
    return `
        <div class="dms-rules">
...
function renderDMSNotesWidget(): string {
...
export function saveDMSNotes(): void {
    const input = $('dms-notes-input') as HTMLTextAreaElement | null;
    if (input) {
 ...
 */
function renderDMSActionsWidget(): string {
    return `
 ...
 */
function renderDMSAttributesWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSSavesWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSSkillsWidget(): string {
    return `
 ...
 */
function renderDMSEconomyWidget(): string {
    return `
 ...
 */
function renderDMSSizesWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSObjectsWidget(): string {
    return `
        <div class="dms-ref-widget">
            <div class="dms-ref-section">
 ...
 */
function renderDMSImprovisedWidget(): string {
    return `
        <div class="dms-ref-widget">
            <div class="dms-ref-section">
 ...
 */
function renderDMSRitualWidget(): string {
    return `
        <div class="dms-ref-widget">
            <div class="dms-ref-section">
 ...
 */
function renderDMSDamageWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSTerrainWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSKnowledgeWidget(): string {
    return `
        <div class="dms-ref-widget">
 ...
 */
function renderDMSTravelWidget(): string {
    return `
        <div class="dms-ref-widget">
            <div class="dms-ref-section">
...
document.addEventListener('click', function(e: MouseEvent) {
    ...
    switch(action) {
        ...
        case 'dms-roll-custom':
            const formulaEl = $('dms-dice-formula') as HTMLInputElement | null;
...
document.addEventListener('change', function(e: Event) {
    const target = (e.target as HTMLElement).closest('[data-action="dms-toggle-widget"]') as HTMLElement | null;
    if (target && target.dataset.widget) {
        toggleDMSWidget(target.dataset.widget);
    }
});

export function dmsRollOnTable(tableId: number): void {
    const table = (D.randomTables || []).find((t: RandomTable) => t.id === tableId);
    if (!table || !table.entries || table.entries.length === 0) return;

    // Calculate total weight
    const totalWeight = table.entries.reduce((sum: number, e: TableEntry) => sum + (e.weight || 1), 0);
...
(window as any).renderDMScreen = renderDMScreen;
(window as any).resetDMScreenLayout = resetDMScreenLayout;
(window as any).switchDMSProfile = switchDMSProfile;
(window as any).saveDMSProfileAs = saveDMSProfileAs;
(window as any).deleteDMSProfile = deleteDMSProfile;
(window as any).toggleDMSProfileDropdown = toggleDMSProfileDropdown;
(window as any).toggleDMSConfigDropdown = toggleDMSConfigDropdown;
(window as any).toggleDMSWidget = toggleDMSWidget;
(window as any).hideDMSWidget = hideDMSWidget;
(window as any).dmsRollDice = dmsRollDice;
(window as any).dmsRollOnTable = dmsRollOnTable;
(window as any).saveDMSNotes = saveDMSNotes;
