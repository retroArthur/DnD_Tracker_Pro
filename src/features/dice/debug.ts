...
const undoStack = (window as any).undoStack;
const redoStack = (window as any).redoStack;
const EventDelegation = (window as any).EventDelegation;
const ENTITY_TYPE_CONFIG = (window as any).ENTITY_TYPE_CONFIG;
const renderEntityLink = (window as any).renderEntityLink;
const ErrorHandler = (window as any).ErrorHandler;
const ENTITY_SCHEMAS = (window as any).ENTITY_SCHEMAS;
const safeJSONParse = (window as any).safeJSONParse;
const safeJSONStringify = (window as any).safeJSONStringify;
const repairDataArrays = (window as any).repairDataArrays;
...
const rollQrefDice = (window as any).rollQrefDice;
const qrefSearch = (window as any).qrefSearch;
const toggleEventLog = (window as any).toggleEventLog;
const clearEventLog = (window as any).clearEventLog;
const parseWikiLinks = (window as any).parseWikiLinks;
const navigateToWikiEntry = (window as any).navigateToWikiEntry;
const addNode = (window as any).addNode;
const saveNodeFromModal = (window as any).saveNodeFromModal;
const selectNodeType = (window as any).selectNodeType;
...
export function clearAllSpells(): void {
...
export function clearMindmap(): void {
    if (!confirm('Wirklich das komplette Netzwerk leeren?')) return;
...
export const clearAllNodes = clearMindmap;
...
export function runAllTests(): TestResult[] {
    ...
    showToast(`🧪 Gesamt: ${passed}/${total} Tests bestanden (${percentage}%)`,
        percentage === 100 ? 'success' : percentage >= 80 ? 'info' : 'warning');

    return allResults;
}

export function runFeatureTests(silent: boolean = false): TestResult[] {
...
export function runNewFeatureTests(silent: boolean = false): TestResult[] {
...
export function runEntitySystemTests(silent: boolean = false): TestResult[] {
    ...
    convenienceMethods.forEach(method => {
        results.push({
            ...
            pass: typeof (EntityLookup as any)[method] === 'function',
            detail: 'Verfügbar',
            category: 'entity-system'
        });
    });
...
export function runErrorHandlingTests(silent: boolean = false): TestResult[] {
...
export function runPerformanceTests(silent: boolean = false): TestResult[] {
    ...
    if (typeof renderAll === 'function') {
        ...
        (window as any).renderPending = false; // Reset für Test
    }
...
export function runUITests(silent: boolean = false): TestResult[] {
    ...
    renderFns.forEach(fn => {
        results.push({
            name: fn,
            pass: typeof (window as any)[fn] === 'function',
            detail: 'Funktion verfügbar',
            category: 'ui'
        });
    });
...
export function generateTestWiki(count: number = 5): void {
...
export function generateTestMindmap(count: number = 10): void {
...
export function testWikiSystem(): void {
    ...
    D.wiki = D.wiki.filter((w: any) => w.id !== 999999);
...
export function testWikiLinks(): void {
...
export function testNetworkSystem(): void {
...
export function testNodeTypes(): void {
    ...
    if (hasNodes) {
        ...
        const usedTypes = [...new Set(D.mindmap.nodes.map((n: any) => n.type).filter(Boolean))];
...
export function testQuickReference(): void {
...
export function testIndexedDB(): void {
...
export async function completeReset(): Promise<void> {
    ...
    try {
        ...
        for (const key in (window as any).D) delete (window as any).D[key];
...
function debugLogAdd(message: string): void {
...
export function showDebugModal(): void {
    updateDebugStats();
...
function updateDebugSystemStatus(): void {
...
export function runValidation(silent: boolean = false): TestResult[] {
    ...
    arrays.forEach(arr => {
        if (Array.isArray(D[arr]) && D[arr].length > 0) {
            const ids = D[arr].map((item: any) => item.id).filter((id: number) => id !== undefined);
...
(window as any).clearAllSpells = clearAllSpells;
(window as any).clearAllWiki = clearAllWiki;
(window as any).clearMindmap = clearMindmap;
(window as any).clearAllNodes = clearAllNodes;
(window as any).runAllTests = runAllTests;
(window as any).runFeatureTests = runFeatureTests;
(window as any).runNewFeatureTests = runNewFeatureTests;
(window as any).runEntitySystemTests = runEntitySystemTests;
...
(window as any).generateTestWiki = generateTestWiki;
(window as any).generateTestMindmap = generateTestMindmap;
(window as any).testWikiSystem = testWikiSystem;
(window as any).testWikiLinks = testWikiLinks;
(window as any).testNetworkSystem = testNetworkSystem;
(window as any).testNodeTypes = testNodeTypes;
(window as any).testQuickReference = testQuickReference;
(window as any).testEventLog = testEventLog;
(window as any).testUndoRedo = testUndoRedo;
...
(window as any).clearDebugLog = clearDebugLog;
(window as any).showDebugModal = showDebugModal;
(window as any).runValidation = runValidation;
(window as any).debugLogAdd = debugLogAdd;
