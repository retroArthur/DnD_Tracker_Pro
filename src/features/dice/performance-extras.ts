...
import { showToast, debounce, formatDate } from '@utils/utilities';
...
const fuzzySearchFields = (window as any).fuzzySearchFields;
const highlightMatch = (window as any).highlightMatch;
const getTypeIcon = (window as any).getTypeIcon;
const timers = (window as any).timers;
const formatTime = (window as any).formatTime;

// Debounced Render für häufige Updates (zusätzlich zu den bei SPELLS definierten)
export const debouncedRenderParty = debounce(() => renderParty(), 100);
export const debouncedRenderQuests = debounce(() => renderQuests(), 100);
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (el.dataset.lazySrc) {
                (el as HTMLImageElement).src = el.dataset.lazySrc;
...
export function showMobileQuickNotes(): void {
    ...
    const textarea = $('mobile-quick-notes') as HTMLTextAreaElement | null;
    if (textarea) textarea.value = D.quickNotes || '';
    showModal('mobile-quicknotes-modal');
    setTimeout(() => ($('mobile-quick-notes') as HTMLTextAreaElement | null)?.focus(), 100);
}

export function saveMobileQuickNotes(): void {
    const textarea = $('mobile-quick-notes') as HTMLTextAreaElement | null;
    const notes = textarea?.value || '';
    D.quickNotes = notes;
    // Sync mit Desktop-Textarea
    const desktopNotes = $('quick-notes') as HTMLTextAreaElement | null;
...
export function showMobileSearch(): void {
    ...
    setTimeout(() => ($('mobile-search-input') as HTMLInputElement | null)?.focus(), 100);
}

export function performMobileSearch(): void {
    const input = $('mobile-search-input') as HTMLInputElement | null;
...
let draggedList: HTMLElement | null = null;

export function handleDragStart(e: DragEvent): void {
    const target = e.target as HTMLElement;
    const item = target.closest('[data-sortable]') as HTMLElement | null;
    if (!item) return;

    draggedItem = item;
    draggedList = item.parentElement as HTMLElement;
...
export function handleDragOver(e: DragEvent): void {
    ...
    const target = targetEl.closest('[data-sortable]') as HTMLElement | null;
...
export function handleDrop(e: DragEvent): void {
    ...
    const items = [...container.querySelectorAll('[data-sortable]')] as HTMLElement[];
    ...
    if (dataKey && D[dataKey]) {
        ...
        const draggedData = dataArray.find((x: any) => x.id === draggedId);
        const targetData = dataArray.find((x: any) => x.id === targetId);
...
export function renderDashboard(): void {
    ...
    const activeQuests = D.quests?.filter((q: any) => q.status !== 'completed')?.length || 0;
    ...
    const partyHealth = D.characters?.map((ch: any) => {
    ...
    // Tracked Quests
    const trackedQuestList = (D.quests || []).filter((q: any) => q.tracked && !q.completed).slice(0, 4);

    // Active timers (if any)
    const activeTimers = typeof timers !== 'undefined' ? timers.filter((t: any) => t.running) : [];
        ...
        <div class="dash-columns">
            ...
            <div class="dash-card">
                ...
                <div class="dash-card-content">
                    ${trackedQuestList.length ? trackedQuestList.map((q: any) => `
            ...
            <div class="dash-card">
                ...
                <div class="dash-card-content">
                    ${recentSessions.length ? recentSessions.map((s: any) => `
...
(window as any).debouncedRenderParty = debouncedRenderParty;
(window as any).debouncedRenderQuests = debouncedRenderQuests;
(window as any).lazyLoadObserver = lazyLoadObserver;
(window as any).toggleMobileNav = toggleMobileNav;
(window as any).showMobileQuickNotes = showMobileQuickNotes;
(window as any).saveMobileQuickNotes = saveMobileQuickNotes;
(window as any).showMobileSearch = showMobileSearch;
(window as any).performMobileSearch = performMobileSearch;
