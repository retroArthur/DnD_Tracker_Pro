...
let _srdSpellsCache: SRDSpell[] | null = null;
...
export function loadSRDSpells(): void {
    const spells = getSRDSpells();

    if (D.spells && D.spells.length > 0) {
        if (!confirm(`${D.spells.length} Zauber vorhanden. SRD-Zauber hinzufügen (keine Duplikate)?`)) return;
    }

    let added = 0;
    const existingNames = new Set((D.spells || []).map((s: any) => s.name.toLowerCase()));
...
(window as any).getSRDSpells = getSRDSpells;
(window as any).SRD_SPELLS = SRD_SPELLS;
(window as any).loadSRDSpells = loadSRDSpells;
