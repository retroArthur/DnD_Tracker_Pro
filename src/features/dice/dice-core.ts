...
let diceHistory: DiceHistoryEntry[] = [];
let diceFormulaHistory: string[] = [];
let lastDiceRoll: DiceRollResult | null = null;
let selectedDamageType: string | null = null;

const SKILLS: { [key: string]: SkillDefinition[] } = {
...
export function rollDiceAnimated(sides: number): void {
    ...
    if (btn) {
        ...
        setTimeout(() => btn.classList.remove('rolling'), (window as any).APP_CONFIG.ANIMATION_SLOW);
    }
...
export function rollCustomDice(): void {
    const notationEl = $('dice-notation') as HTMLInputElement | null;
    const multiEl = $('dice-multi') as HTMLInputElement | null;
...
export function rerollLast(): void {
    if (!lastDiceRoll) {
...
export function parseDiceNotation(notation: string): ParseDiceNotationResult | null {
...
export function rollAdvantage(): void {
...
export function rollDisadvantage(): void {
...
export function rollStats(): void {
    const stats: number[] = [];
    const allRolls: number[] = [];
    for (let i = 0; i < 6; i++) {
...
export function rollCritDamage(): void {
    const notationEl = $('dice-notation') as HTMLInputElement | null;
    if (!notationEl) return;
...
export function rollAttack(withAdvantage: boolean = false): void {
    const targetACEl = $('target-ac') as HTMLInputElement | null;
    const attackBonusEl = $('attack-bonus') as HTMLInputElement | null;
...
export function rollSavingThrow(): void {
    const dcEl = $('save-dc') as HTMLInputElement | null;
    const attrEl = $('save-attr') as HTMLSelectElement | null;
    const modEl = $('save-mod') as HTMLInputElement | null;
...
export function rollGroupPerception(): void {
    ...
    const results: GroupPerceptionResult[] = [];
    D.characters.forEach((ch: CharacterEntity) => {
...
export function displayDiceResult(
    ...
    if (heroContainer && heroResult) {
        ...
        if (extra) {
            if (extraHtml) {
                ...
                (extra as HTMLElement).style.display = 'block';
            } else {
                (extra as HTMLElement).style.display = 'none';
            }
        }
...
function createConfetti(container: Element): void {
...
export function toggleDamageType(chip: HTMLElement): void {
...
export function addToDiceHistory(notation: string, result: string | number, rolls: (number | string)[]): void {
    const dmgType = selectedDamageType ? ` (${selectedDamageType})` : '';
...
export function addToFormulaHistory(formula: string): void {
...
export function renderFormulaHistory(): void {
    const c = $('dice-formula-history');
    if (!c) return;
    c.innerHTML = diceFormulaHistory.map(f =>
        `<span class="dice-formula-chip" data-action="set-dice-formula" data-value="${f}">${f}</span>`
    ).join('');
}

export function renderDiceHistory(): void {
...
export function updateDiceCharSelect(): void {
    const select = $('dice-char-select') as HTMLSelectElement | null;
    if (!select) return;

    const D = (window as any).D;
    select.innerHTML = '<option value="">— Charakter —</option>' +
        (D.characters || []).map((ch: CharacterEntity) => `<option value="${ch.id}">${esc(ch.name)}</option>`).join('');
}

export function updateDiceCharStats(): void {
    const selectEl = $('dice-char-select') as HTMLSelectElement | null;
    if (!selectEl) return;

    const charId = parseInt(selectEl.value);
    const D = (window as any).D;
    const ch = D.characters?.find((c: CharacterEntity) => c.id === charId);

    const attrs = ch?.attributes || { str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10 };
    const saves = ch?.saveProficiencies || {};
    const profBonus = ch?.proficiencyBonus || (window as any).getProficiencyBonus(ch?.level || 1);

    // Update attribute mods
    (['str', 'dex', 'con', 'int', 'wis', 'cha'] as const).forEach(attr => {
        ...
        if (saveEl) saveEl.textContent = (saveMod >= 0 ? '+' : '') + saveMod + (saves[attr] ? ' ●' : '');
    });
...
export function rollAttrCheck(attr: string): void {
    const selectEl = $('dice-char-select') as HTMLSelectElement | null;
    if (!selectEl) return;
    ...
    const val = ch?.attributes?.[attr as keyof CharacterEntity['attributes']] || 10;
    const mod = Math.floor((val - 10) / 2);
...
export function rollCharSave(attr: string): void {
    const selectEl = $('dice-char-select') as HTMLSelectElement | null;
    if (!selectEl) return;
    ...
    const mod = Math.floor((val - 10) / 2);
    const profBonus = ch?.proficiencyBonus || (window as any).getProficiencyBonus(ch?.level || 1);
    const isProficient = ch?.saveProficiencies?.[attr as keyof CharacterEntity['saveProficiencies']];
...
export function rollSkillCheck(skill: string, mod: number, skillName: string): void {
    const selectEl = $('dice-char-select') as HTMLSelectElement | null;
    if (!selectEl) return;
...
export function rollCharInitiative(): void {
    const selectEl = $('dice-char-select') as HTMLSelectElement | null;
    if (!selectEl) return;
...
export function rollFloatingDice(sides: number): void {
...
export function rollFloatingAdvantage(): void {
    const result = parseDiceNotation('2d20kh1');
    if (!result) return;
...
export function rollFloatingDisadvantage(): void {
    const result = parseDiceNotation('2d20kl1');
    if (!result) return;
...
export function rollFloatingCustom(): void {
    const notationEl = $('fdp-notation') as HTMLInputElement | null;
    if (!notationEl) return;
...
export function rerollFloating(notation: string): void {
    if (notation === 'Vorteil') {
...
(window as any).rollDiceAnimated = rollDiceAnimated;
(window as any).rollDice = rollDice;
(window as any).rollCustomDice = rollCustomDice;
(window as any).rollMultiple = rollMultiple;
(window as any).rerollLast = rerollLast;
(window as any).parseDiceNotation = parseDiceNotation;
(window as any).rollAdvantage = rollAdvantage;
(window as any).rollDisadvantage = rollDisadvantage;
(window as any).rollStats = rollStats;
(window as any).rollCritDamage = rollCritDamage;
(window as any).flipCoin = flipCoin;
(window as any).rollAttack = rollAttack;
(window as any).rollSavingThrow = rollSavingThrow;
(window as any).rollGroupPerception = rollGroupPerception;
(window as any).displayDiceResult = displayDiceResult;
(window as any).toggleDamageType = toggleDamageType;
(window as any).addToDiceHistory = addToDiceHistory;
(window as any).addToFormulaHistory = addToFormulaHistory;
(window as any).renderFormulaHistory = renderFormulaHistory;
(window as any).renderDiceHistory = renderDiceHistory;
(window as any).clearDiceHistory = clearDiceHistory;
(window as any).updateDiceCharSelect = updateDiceCharSelect;
(window as any).updateDiceCharStats = updateDiceCharStats;
(window as any).rollAttrCheck = rollAttrCheck;
(window as any).rollCharSave = rollCharSave;
(window as any).rollSkillCheck = rollSkillCheck;
(window as any).rollCharInitiative = rollCharInitiative;
(window as any).toggleFloatingDice = toggleFloatingDice;
(window as any).rollFloatingDice = rollFloatingDice;
(window as any).rollFloatingAdvantage = rollFloatingAdvantage;
(window as any).rollFloatingDisadvantage = rollFloatingDisadvantage;
(window as any).rollFloatingCustom = rollFloatingCustom;
(window as any).renderFloatingHistory = renderFloatingHistory;
(window as any).rerollFloating = rerollFloating;
