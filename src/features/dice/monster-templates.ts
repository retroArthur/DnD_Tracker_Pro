...
let _monsterTemplatesCache: Record<string, MonsterTemplate> | null = null;

export function getMonsterTemplates(): Record<string, MonsterTemplate> {
...
const MONSTER_TEMPLATES = new Proxy({}, {
    get(target: any, prop: string | symbol) {
        return getMonsterTemplates()[prop as string];
    }
});

export function loadMonsterTemplate(key: string): void {
    const templates = getMonsterTemplates();
    const t = templates[key];
    if (!t) return;

    ($('enc-name') as HTMLInputElement).value = t.name;
    ($('enc-creature-type') as HTMLInputElement).value = t.creatureType;
    ($('enc-cr') as HTMLInputElement).value = t.cr;
    ($('enc-ac') as HTMLInputElement).value = String(t.ac);
    ($('enc-init') as HTMLInputElement).value = String(t.init);
    ($('enc-hp') as HTMLInputElement).value = String(t.hp);

    // Load speed (support both old string format and new object format)
    if (typeof t.speed === 'object' && t.speed !== null) {
        ($('enc-speed-walk') as HTMLInputElement).value = t.speed.walk || '';
        ($('enc-speed-climb') as HTMLInputElement).value = t.speed.climb || '';
        ($('enc-speed-swim') as HTMLInputElement).value = t.speed.swim || '';
        ($('enc-speed-fly') as HTMLInputElement).value = t.speed.fly || '';
        ($('enc-speed-burrow') as HTMLInputElement).value = t.speed.burrow || '';
    } else {
        // Old format: single string -> put in walk
        ($('enc-speed-walk') as HTMLInputElement).value = (t.speed as any) || '';
    ...
    ($('enc-perception') as HTMLInputElement).value = String(parseInt(String(t.perception)) || 0);
    ($('enc-str') as HTMLInputElement).value = t.str;
    ($('enc-dex') as HTMLInputElement).value = t.dex;
    ($('enc-con') as HTMLInputElement).value = t.con;
    ($('enc-int') as HTMLInputElement).value = t.int;
    ($('enc-wis') as HTMLInputElement).value = t.wis;
    ($('enc-cha') as HTMLInputElement).value = t.cha;
    ($('enc-traits') as HTMLElement).innerHTML = sanitizeHTML(t.traits) || '';
    ($('enc-equipment') as HTMLElement).innerHTML = sanitizeHTML(t.equipment) || '';
    ($('enc-actions') as HTMLElement).innerHTML = sanitizeHTML(t.actions) || '';

    // Set languages
    const langSelect = $('enc-languages') as HTMLSelectElement;
    ...
    const form = $('enc-form') as HTMLElement;
    form.classList.add('open');
    const icon = $('enc-form-icon') as HTMLElement;
    icon.textContent = '▲';
...
(window as any).getMonsterTemplates = getMonsterTemplates;
(window as any).MONSTER_TEMPLATES = MONSTER_TEMPLATES;
(window as any).loadMonsterTemplate = loadMonsterTemplate;
