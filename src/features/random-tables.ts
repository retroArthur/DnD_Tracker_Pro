...
const DEFAULT_DICE_TYPE: number = 6;
const DICE_TYPES: readonly number[] = Object.freeze([4, 6, 8, 10, 12, 20, 100]);
const MAX_RANGE_SIZE: number = 100; // DoS-Schutz: Maximale Anzahl von Werten in einem Range
...
function initRandomTables(): void {
...
function getDefaultRandomTables(): RandomTable[] {
    return [
        {
            id: 1,
 ...
 */
function rollWeightedEntry(table: RandomTable | null | undefined): RollResult | null {
 ...
 */
function parseRange(rangeStr: string, maxValue: number = MAX_RANGE_SIZE): number[] {
...
export function renderRandomTables(): void {
...
export function selectTable(id: number): void {
    selectedTableId = id;
    renderRandomTables();
    showTablePreview(id);
}

function showTablePreview(id: number): void {
    const preview = $('random-table-preview');
    if (!preview) return;

    const D = (window as any).D;
    const table: RandomTable | undefined = D.randomTables?.find((t: RandomTable) => t.id === id);
    if (!table) {
...
export function rollOnTable(id: number): void {
    const D = (window as any).D;
    const table: RandomTable | undefined = D.randomTables?.find((t: RandomTable) => t.id === id);
...
export function showTableModal(id: number | null = null): void {
    const D = (window as any).D;
    const table: RandomTable | undefined = id ? D.randomTables?.find((t: RandomTable) => t.id === id) : undefined;
    const diceType = table?.diceType ?? DEFAULT_DICE_TYPE;
    ...
    if (!modal) {
        ...
        (modal as HTMLElement).onclick = (e: MouseEvent) => {
            if (e.target === modal) hideModal('table-modal');
        };
    ...
    ($('table-name') as HTMLInputElement)?.focus();
    updateRangeHint();
}

export function selectDiceType(diceType: number): void {
    const input = $('table-dice-type') as HTMLInputElement;
    if (input) input.value = String(diceType);

    // Update button states
    document.querySelectorAll('.rt-dice-btn').forEach(btn => {
        const btnDice = parseInt((btn as HTMLElement).dataset.dice || '0');
...
function renderTableEntryRow(index: number, entry: RandomTableEntry = { range: '', text: '' }): string {
 ...
 */
export function updateRangeHint(): void {
    const hint = $('rt-range-hint');
    if (!hint) return;

    const diceTypeInput = $('table-dice-type') as HTMLInputElement;
    ...
    rows.forEach(row => {
        const rangeInput = row.querySelector('.rt-entry-range') as HTMLInputElement;
 ...
 */
export function fillRemainingRanges(): void {
    const diceTypeInput = $('table-dice-type') as HTMLInputElement;
    const diceType = parseInt(diceTypeInput?.value || String(DEFAULT_DICE_TYPE));
    ...
    while (available.length > 0) {
        ...
        if (newRows) {
            ...
            const rangeInput = lastRow?.querySelector('.rt-entry-range') as HTMLInputElement;
            if (rangeInput) {
                rangeInput.value = String(available.shift());
                filled++;
            }
...
export function addTableEntry(): void {
...
export function removeTableEntry(index: number): void {
    const list = $('table-entries');
    ...
    Array.from(list.children).forEach((row, i) => {
        ...
        const btn = row.querySelector('.btn-danger') as HTMLElement;
        if (btn) btn.onclick = () => removeTableEntry(i);
    });
}

export function saveTable(): void {
    const nameInput = $('table-name') as HTMLInputElement;
    ...
    const iconInput = $('table-icon') as HTMLInputElement;
    ...
    const diceTypeInput = $('table-dice-type') as HTMLInputElement;
    const diceType = parseInt(diceTypeInput?.value || String(DEFAULT_DICE_TYPE));
    const editIdInput = $('table-edit-id') as HTMLInputElement;
    ...
    rows?.forEach(row => {
        const rangeInput = row.querySelector('.rt-entry-range') as HTMLInputElement;
        const textInput = row.querySelector('.rt-entry-text') as HTMLInputElement;
    ...
    initRandomTables();
    const D = (window as any).D;
    (window as any).pushUndo(editId ? 'Tabelle bearbeitet' : 'Tabelle erstellt');

    if (editId) {
        // Update
        const idx = D.randomTables.findIndex((t: RandomTable) => t.id === parseInt(editId));
        if (idx > -1) {
...
export function deleteTable(id: number): void {
    ...
    const D = (window as any).D;
    (window as any).pushUndo('Tabelle gelöscht');
    D.randomTables = (D.randomTables || []).filter((t: RandomTable) => t.id !== id);
...
export function showGeneratorModal(): void {
    initRandomTables();
    const D = (window as any).D;
    const tables: RandomTable[] = D.randomTables || [];
    ...
    if (!modal) {
        ...
        (modal as HTMLElement).onclick = (e: MouseEvent) => {
            if (e.target === modal) hideModal('generator-modal');
        };
        document.body.appendChild(modal);
    } else {
        const modalContent = modal.querySelector('.modal');
...
export function rollOnTableAndShow(id: number): void {
    const D = (window as any).D;
    const table: RandomTable | undefined = D.randomTables?.find((t: RandomTable) => t.id === id);
    const rollResult = rollWeightedEntry(table);
...
export function deleteTableAndRefresh(id: number): void {
    if (!confirm('Tabelle wirklich löschen?')) return;
...
export function quickRandomRoll(): void {
    initRandomTables();
    const D = (window as any).D;
    ...
    const content = `
        <div style="padding: 20px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px;">
                ${D.randomTables.map((t: RandomTable) => `
    ...
    if (!modal) {
        ...
        (modal as HTMLElement).onclick = (e: MouseEvent) => {
            if (e.target === modal) hideModal('quick-roll-modal');
        };
        document.body.appendChild(modal);
    } else {
        const modalContent = modal.querySelector('.modal');
...
(window as any).renderRandomTables = renderRandomTables;
(window as any).selectTable = selectTable;
(window as any).rollOnTable = rollOnTable;
(window as any).showTableModal = showTableModal;
(window as any).selectDiceType = selectDiceType;
(window as any).updateRangeHint = updateRangeHint;
(window as any).fillRemainingRanges = fillRemainingRanges;
(window as any).addTableEntry = addTableEntry;
(window as any).removeTableEntry = removeTableEntry;
(window as any).saveTable = saveTable;
(window as any).deleteTable = deleteTable;
(window as any).showGeneratorModal = showGeneratorModal;
(window as any).rollOnTableAndShow = rollOnTableAndShow;
(window as any).deleteTableAndRefresh = deleteTableAndRefresh;
(window as any).quickRandomRoll = quickRandomRoll;
