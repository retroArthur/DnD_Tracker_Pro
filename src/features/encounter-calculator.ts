...
const XP_THRESHOLDS: { [level: number]: XPThreshold } = {
...
const CR_TO_XP: { [cr: string]: number } = {
    "0": 10,
    "1/8": 25,
    "1/4": 50,
    "1/2": 100,
...
const ENCOUNTER_MULTIPLIERS: EncounterMultiplier[] = [
...
const TERRAIN_MODIFIERS: TerrainModifier[] = [
...
let calculatorParty: PartyMember[] = []; // { level: number, count: number }
let calculatorMonsters: Monster[] = []; // { cr: string, count: number, name?: string }
let calculatorTerrain: string = 'normal'; // Terrain modifier ID
let calculatorLairActions: boolean = false; // Has lair actions (+1 effective CR)
let calculatorTargetDifficulty: number = 1; // 0=easy, 1=medium, 2=hard, 3=deadly

// XP-Budget-Slider Handling
const DIFFICULTY_LEVELS: string[] = ['easy', 'medium', 'hard', 'deadly'];
const DIFFICULTY_LABELS: string[] = ['Easy', 'Medium', 'Hard', 'Deadly'];

let monsterPreviewTimer: number | null = null;

// ============================================================
// BUDGET FUNCTIONS
// ============================================================

export function onBudgetSliderChange(value: string): void {
...
function updateBudgetDisplay(): void {
    ...
    const targetXP = thresholds[targetLevel as keyof PartyThresholds] || 0;
...
export function applyBudgetTarget(): void {
...
export function saveMonsterFavorite(): void {
    if (calculatorMonsters.length === 0) {
        showToast('Füge zuerst Monster hinzu');
        return;
    }
    ...
    (window as any).saveImmediate();
    showToast(`Vorlage "${name}" gespeichert`);
    renderCalculator();
}

export function loadMonsterFavorite(id: number): void {
    const D = (window as any).D;
    const fav = (D.monsterFavorites || []).find((f: MonsterFavorite) => f.id === id);
    if (!fav) return;
...
export function deleteMonsterFavorite(id: number): void {
    const D = (window as any).D;
    if (!D.monsterFavorites) return;

    const idx = D.monsterFavorites.findIndex((f: MonsterFavorite) => f.id === id);
    if (idx === -1) return;
...
export function toggleFavoritesDropdown(): void {
...
function closeFavoritesDropdown(): void {
    const dropdown = $('calc-favorites-dropdown');
    if (dropdown) dropdown.classList.remove('show');
}

function renderFavoritesDropdown(): void {
    const dropdown = $('calc-favorites-dropdown');
    if (!dropdown) return;

    const D = (window as any).D;
    const favs: MonsterFavorite[] = D.monsterFavorites || [];
...
export function addPartyLevel(): void {
    const levelInput = $('calc-party-level') as HTMLInputElement;
    const countInput = $('calc-party-count') as HTMLInputElement;
...
export function removePartyLevel(index: number): void {
    calculatorParty.splice(index, 1);
    renderCalculator();
    recalculateEncounter();
}

export function loadPartyFromCharacters(): void {
    const D = (window as any).D;
...
export function addMonster(): void {
    const crInput = $('calc-monster-cr') as HTMLInputElement;
    const countInput = $('calc-monster-count') as HTMLInputElement;
    const nameInput = $('calc-monster-name') as HTMLInputElement;
...
export function removeMonster(index: number): void {
...
export function updateMonsterPreview(): void {
...
function clearMonsterPreview(): void {
    const previewDiv = $('calc-monster-preview');
    if (previewDiv) previewDiv.innerHTML = '';
}

// Debounced Preview-Update
export function scheduleMonsterPreview(): void {
...
function calculatePartyThresholds(): PartyThresholds {
...
function calculateMonsterXP(): MonsterXP {
    if (calculatorMonsters.length === 0) {
...
function getDifficulty(adjustedXP: number, thresholds: PartyThresholds): Difficulty {
...
function getMultiplierExplanation(totalMonsters: number, partySize: number, multiplier: number): string {
    // Finde Basis-Bracket
...
function recalculateEncounter(): void {
    const thresholds = calculatePartyThresholds();
...
export function quickAdjustDifficulty(direction: string): void {
    if (calculatorMonsters.length === 0) {
...
function calculateOptimalMonsterCount(targetDifficultyLevel: string): void {
    if (calculatorMonsters.length === 0) {
        showToast('Keine Monster vorhanden');
        return;
    }
...
export function showDifficultySelector(): void {
    if (calculatorMonsters.length === 0) {
        showToast('Füge zuerst Monster hinzu');
        return;
    }
...
export function saveAsEncounter(): void {
    if (calculatorMonsters.length === 0) {
...
Difficulty: ${difficulty.label}`
    ...
    if (confirm('Zu Encounters wechseln?')) {
        hideCalculatorModal();
        (window as any).switchView('encounter');
        if (typeof (window as any).renderEncounters === 'function') (window as any).renderEncounters();
    }
}
...
export function addCalculatorToInitiative(): void {
    if (calculatorMonsters.length === 0) {
        showToast('Keine Monster vorhanden');
        return;
    }
    ...
    calculatorMonsters.forEach(monster => {
        ...
        const template = (typeof D !== 'undefined' && D.encounters)
            ? D.encounters.find((e: any) => e.cr === monster.cr || e.CR === monster.cr)
            : null;
    ...
    if (calculatorLairActions) {
        ...
        const existingLair = D.initiative.combatants.find((c: any) => c.type === 'lair');
        if (!existingLair) {
            D.initiative.combatants.push({
                id: nextId('combatants'),
                name: '🏰 Lair Action',
    ...
    (window as any).save();
    hideCalculatorModal();
    (window as any).switchView('initiative');
    if (typeof (window as any).renderInit === 'function') (window as any).renderInit();
    showToast(`${addedCount} Monster zur Initiative hinzugefügt`);
}

// Default HP by CR (rough estimates)
function getDefaultHPForCR(cr: string): number {
    const hpByCR: { [cr: string]: number } = {
...
export function showCalculatorModal(): void {
...
export function hideCalculatorModal(): void {
    const modal = $('calculator-modal');
    if (modal) {
...
function renderCalculatorModal(): void {
...
export function setCalculatorTerrain(terrainId: string): void {
    calculatorTerrain = terrainId;
    renderCalculatorModal();
    recalculateEncounter();
}

export function toggleCalculatorLair(): void {
...
export function showEncounterImport(): void {
    const D = (window as any).D;
...
export function importEncounterMonsters(encId: number): void {
    const D = (window as any).D;
    const encounter = D.encounters.find((e: any) => e.id === encId || e.id === parseInt(String(encId)));
    ...
    const countInput = $('import-count-' + encId) as HTMLInputElement;
...
export function renderEncounterCalculator(): void {
...
(window as any).onBudgetSliderChange = onBudgetSliderChange;
(window as any).applyBudgetTarget = applyBudgetTarget;
(window as any).saveMonsterFavorite = saveMonsterFavorite;
(window as any).loadMonsterFavorite = loadMonsterFavorite;
(window as any).deleteMonsterFavorite = deleteMonsterFavorite;
(window as any).toggleFavoritesDropdown = toggleFavoritesDropdown;
(window as any).addPartyLevel = addPartyLevel;
(window as any).removePartyLevel = removePartyLevel;
(window as any).loadPartyFromCharacters = loadPartyFromCharacters;
(window as any).clearParty = clearParty;
(window as any).addMonster = addMonster;
(window as any).removeMonster = removeMonster;
(window as any).clearMonsters = clearMonsters;
(window as any).updateMonsterPreview = updateMonsterPreview;
(window as any).scheduleMonsterPreview = scheduleMonsterPreview;
(window as any).quickAdjustDifficulty = quickAdjustDifficulty;
(window as any).showDifficultySelector = showDifficultySelector;
(window as any).saveAsEncounter = saveAsEncounter;
(window as any).addCalculatorToInitiative = addCalculatorToInitiative;
(window as any).showCalculatorModal = showCalculatorModal;
(window as any).hideCalculatorModal = hideCalculatorModal;
(window as any).setCalculatorTerrain = setCalculatorTerrain;
(window as any).toggleCalculatorLair = toggleCalculatorLair;
(window as any).showEncounterImport = showEncounterImport;
(window as any).importEncounterMonsters = importEncounterMonsters;
(window as any).renderEncounterCalculator = renderEncounterCalculator;
