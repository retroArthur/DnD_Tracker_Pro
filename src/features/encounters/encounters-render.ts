...
// State
let selectedEncounterId: number | null = null;
let currentEncFilter: string = 'all';

// Creature type icons
const ENC_ICONS: Record<string, string> = {
    'aberration': '👁️',
    'bestie': '🐾',
    'himmlisch': '👼',
    'konstrukt': '🤖',
...
function formatEncSpeed(speed: any, html: boolean = false): string {
    if (!speed) return '—';
    ...
    const icons: Record<string, string> = { walk: '🚶', climb: '🧗', swim: '🏊', fly: '🦅', burrow: '⛏️' };
    const labels: Record<string, string> = { walk: '', climb: 'Klettern', swim: 'Schwimmen', fly: 'Fliegen', burrow: 'Graben' };

    const parts: SpeedParts[] = [];
...
function getEncounterIcon(enc: any): string {
...
export function renderEncounters(): void {
        ...
        filterContainer.innerHTML = `
            ...
            ${types.length > 5 ? `
                <select class="enc-filter-select" data-on-change="setEncFilter">
                    ...
                    ${types.slice(5).map((type: string) => `<option value="${esc(type)}">${esc(type)}</option>`).join('')}
                </select>
            ` : ''}
        `;
    }
    ...
    const searchEl = $('enc-search') as HTMLInputElement | null;
    const search = (searchEl?.value || '').toLowerCase();
    let encounters = [...(D.encounters || [])];

    // Apply type filter
    if (currentEncFilter !== 'all') {
        encounters = encounters.filter((e: any) => e.creatureType === currentEncFilter);
    }
    ...
    // Render list items
    listContainer.innerHTML = encounters.map((enc: any) => renderEncounterItem(enc)).join('');

    // Auto-select first if none selected
    if (!selectedEncounterId || !encounters.find((e: any) => e.id === selectedEncounterId)) {
...
function parseCR(cr: any): number {
    if (!cr) return 0;
...
function getEncounterDifficulty(cr: any): DifficultyResult | null {
    const D = (window as any).D;
    if (!D.characters || D.characters.length === 0) return null;

    // Get party average level
    const avgLevel = Math.round(D.characters.reduce((sum: number, c: any) => sum + (c.level || 1), 0) / D.characters.length);
...
function renderEncounterItem(enc: any): string {
...
export function selectEncounter(id: number | string, scroll: boolean = true): void {
    selectedEncounterId = typeof id === 'string' ? parseInt(id) : id;

    // Update selection in list
    document.querySelectorAll('.enc-item').forEach(el => {
        const element = el as HTMLElement;
        element.classList.toggle('selected', element.dataset.id === String(selectedEncounterId));
    });
    ...
    if (scroll) {
        const item = document.querySelector(`.enc-item[data-id="${selectedEncounterId}"]`);
        if (item) {
...
export function showEncounterDetail(id: number | string): void {
    ...
    const resistancesHtml = (enc.resistances || []).map((r: string) => `<span class="enc-res-tag res">${esc(r)}</span>`).join('');
    const immunitiesHtml = (enc.immunities || []).map((i: string) => `<span class="enc-res-tag imm">${esc(i)}</span>`).join('');
    const condImmunitiesHtml = (enc.conditionImmunities || []).map((c: string) => `<span class="enc-res-tag cond">${esc(c)}</span>`).join('');
...
export function clearEncounterDetail(): void {
    const panel = $('enc-detail-panel');
    if (panel) {
        panel.innerHTML = `
...
export function toggleEncounter(id: number | string): void {
    ...
    setTimeout(() => {
        const item = document.querySelector(`.enc-item[data-id="${selectedEncounterId}"]`) as HTMLElement | null;
        if (item) {
...
export function toggleEncounterCard(id: number | string): void {
    selectEncounter(id);
}

// Show encounter form (opens collapsible or could be modal)
export function showEncForm(): void {
    const cancelEncEdit = (window as any).cancelEncEdit;
    cancelEncEdit();
...
(window as any).selectedEncounterId = selectedEncounterId;
(window as any).renderEncounters = renderEncounters;
(window as any).selectEncounter = selectEncounter;
(window as any).showEncounterDetail = showEncounterDetail;
(window as any).clearEncounterDetail = clearEncounterDetail;
(window as any).setEncFilter = setEncFilter;
(window as any).toggleEncounter = toggleEncounter;
(window as any).toggleEncounterCard = toggleEncounterCard;
(window as any).showEncForm = showEncForm;
