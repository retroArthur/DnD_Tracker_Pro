...
export function renderQuests(): void {
    ...
    const renderDashboard = (window as any).renderDashboard;

    const c = $('quests-list');
    if (!c) return;

    const activeOnlyEl = $('quest-filter-active') as HTMLInputElement | null;
    const activeOnly = activeOnlyEl?.checked || false;

    const searchEl = $('quest-search') as HTMLInputElement | null;
    const search = (searchEl?.value || '').toLowerCase();

    let quests = D.quests;

    if (activeOnly) quests = quests.filter((q: any) => !q.completed);
    if (search) quests = quests.filter((q: any) =>
    ...
    c.innerHTML = quests.map((q: any) => renderQuestItem(q)).join('');
}

function renderQuestItem(q: any): string {
    const renderEntityLinks = (window as any).renderEntityLinks;
    const renderTagsBar = (window as any).renderTagsBar;
    ...
    if (q.rewardItems?.length) rewardParts.push(q.rewardItems.map((i: any) => i.name).join(', '));
...
export function toggleQuest(id: number | string): void {
    const element = $(`quest-${id}`);
    element?.classList.toggle('expanded');
}

export function toggleQuestTracked(id: number | string): void {
    const renderDashboard = (window as any).renderDashboard;
...
export function toggleQuestStatus(id: number | string, type: string): void {
    const q = EntityLookup.quest(id);
    if (!q) return;
...
(window as any).renderQuests = renderQuests;
(window as any).toggleQuest = toggleQuest;
(window as any).toggleQuestTracked = toggleQuestTracked;
(window as any).toggleQuestStatus = toggleQuestStatus;
