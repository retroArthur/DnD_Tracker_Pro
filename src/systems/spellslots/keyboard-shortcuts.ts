...
import { saveImmediate } from './persistence';
import { switchView, showModal, toggleCollapse } from './navigation';
    ...
    document.addEventListener('keydown', (e: KeyboardEvent) => {
        ...
        if (e.key === 'Escape') {
            ...
            if (shortcutsOverlay?.classList.contains('show')) {
                const hideShortcutsOverlay = (window as any).hideShortcutsOverlay;
        ...
        if (e.key === '?' || (e.shiftKey && e.key === '/')) {
            e.preventDefault();
            const showShortcutsOverlay = (window as any).showShortcutsOverlay;
        ...
        if (e.key === 't' && !e.ctrlKey && !e.metaKey) {
            e.preventDefault();
            const toggleSessionTimer = (window as any).toggleSessionTimer;
        ...
        if (e.altKey && e.key >= '1' && e.key <= '9') {
            const dice: Record<string, number> = { '1': 12, '2': 20, '4': 4, '6': 6, '8': 8 };
            if (dice[e.key]) {
                e.preventDefault();
        ...
        if (e.key === ' ' && document.querySelector('#view-initiative.active')) {
            e.preventDefault();
            const nextTurn = (window as any).nextTurn;
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && document.querySelector('#view-network.active')) {
            e.preventDefault();
            const deleteSelectedNode = (window as any).deleteSelectedNode;
        ...
        if (e.key === 'N' && e.shiftKey && document.querySelector('#view-initiative.active')) {
            e.preventDefault();
            const nextEncounterRound = (window as any).nextEncounterRound;
        ...
        if (e.key === 'p' && document.querySelector('#view-initiative.active')) {
            e.preventDefault();
            const prevTurn = (window as any).prevTurn;
