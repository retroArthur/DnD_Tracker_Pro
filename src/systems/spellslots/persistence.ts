...
export async function saveImmediate(): Promise<void> {
    ...
    const updateSaveIndicator = (window as any).updateSaveIndicator;
    const broadcastSave = (window as any).broadcastSave;
    ...
    try {
        ...
        if (dataSizeMB > 2) {
            saveToIndexedDBFallback(key, dataString).catch(e =>
                console.log('[IDB Backup] Optional backup failed:', e)
            );
        }
    } catch(e) {
...
async function saveToIndexedDBFallback(key: string, dataString: string): Promise<any> {
    const initIndexedDB = (window as any).initIndexedDB;
    let idb = (window as any).idb;
    if (!idb) await initIndexedDB();
    idb = (window as any).idb; // Reload after init
...
export async function loadFromIndexedDBFallback(key: string): Promise<string> {
    const initIndexedDB = (window as any).initIndexedDB;
    let idb = (window as any).idb;
    if (!idb) await initIndexedDB();
    idb = (window as any).idb; // Reload after init
