...
export function showHpCalculator(type: string, id: number): void {
    const getEntityByTypeAndId = (window as any).getEntityByTypeAndId;
    const entity = getEntityByTypeAndId(type, id);
    if (!entity) return;

    const typeEl = $('hp-calc-type') as HTMLInputElement;
    const idEl = $('hp-calc-id') as HTMLInputElement;
    const titleEl = $('hp-calc-title');
    const currentEl = $('hp-calc-current');
    const valueEl = $('hp-calc-value') as HTMLInputElement;
    ...
    if (currentEl) currentEl.textContent = `${currentHp} / ${maxHp}${tempHp ? ` (+${tempHp})` : ''}`;
...
export function applyHpChange(action: HpAction): void {
    const typeEl = $('hp-calc-type') as HTMLInputElement;
    const idEl = $('hp-calc-id') as HTMLInputElement;
    const valueEl = $('hp-calc-value') as HTMLInputElement;
    ...
    const currentEl = $('hp-calc-current');
    if (currentEl) currentEl.textContent = `${newCurrent} / ${newMax}${entity.tempHp ? ` (+${entity.tempHp})` : ''}`;
