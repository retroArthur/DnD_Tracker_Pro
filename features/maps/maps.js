...
// HELPERS
// ============================================================
// saveImmediate() defined in systems/spellslots/persistence.js
// ============================================================
// STATE
// ============================================================
let mapZoom = 1;
...
function zoomMap(factor) {
    const MAP_CONSTANTS = window.MAP_CONSTANTS;
    mapZoom *= factor;
...
function initMapPanning() {
    ...
    viewport.addEventListener('mouseup', function (_e) {
        if (mapIsPanning) {
            mapIsPanning = false;
    ...
    viewport.addEventListener('mouseleave', function (_e) {
        if (mapIsPanning) {
            mapIsPanning = false;
            viewport.classList.remove('panning');
        }
...
function editMarker(id) {
    ...
    if (typeEl)
        typeEl.value = marker.type || 'poi';
    if (shapeEl)
        shapeEl.value = marker.shape || 'circle';
    if (noteEl)
        noteEl.value = marker.note || '';
    ...
    if (modalTitle)
        modalTitle.textContent = 'Marker bearbeiten';
    if (placementHint)
...
function handleMapClick(event) {
    if (!markerPlacementMode || !pendingMarker)
        return;
    const map = getCurrentMap();
    if (!map)
        return;
...
function renderMapMarkers() {
    ...
    map.markers.forEach(m => {
        ...
        if (m.linkedLocation || m.linkedNpc || m.linkedQuest || m.linkedEncounter) {
            ...
            if (m.linkedLocation)
                linksHtml += `<span class="tooltip-link" data-action="navigate-entity" data-type="location" data-id="${m.linkedLocation}">📍 Ort</span>`;
            if (m.linkedNpc)
                linksHtml += `<span class="tooltip-link" data-action="navigate-entity" data-type="npc" data-id="${m.linkedNpc}">🧑 NPC</span>`;
            if (m.linkedQuest)
                linksHtml += `<span class="tooltip-link" data-action="navigate-entity" data-type="quest" data-id="${m.linkedQuest}">📜 Quest</span>`;
            if (m.linkedEncounter)
                linksHtml += `<span class="tooltip-link" data-action="navigate-entity" data-type="encounter" data-id="${m.linkedEncounter}">⚔️ Encounter</span>`;
            linksHtml += '</div>';
        }
        marker.innerHTML = `
...
function initMapKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        if (!mapsView?.classList.contains('active'))
            return;
...
function toggleMapGrid() {
    ...
    if (btn)
        btn.classList.toggle('active', showMapGrid);
    updateMapGrid();
}
...
function toggleMapFog() {
    ...
    if (btn)
        btn.classList.toggle('active', showMapFog);
...
function toggleMapConnections() {
    ...
    if (btn)
        btn.classList.toggle('active', showMapConnections);
...
function promptCalibrationDistance(pixelDist) {
    ...
    const input = prompt('Gib die bekannte Entfernung zwischen den zwei Punkten ein:\n' +
...
function loadFogFromMap() {
    ...
    if (map.fogRevealed && Array.isArray(map.fogRevealed)) {
        ...
        map.fogRevealed.forEach(area => {
            fogCtx.beginPath();
            fogCtx.arc((area.x / 100) * fogCanvas.width, (area.y / 100) * fogCanvas.height, area.radius, 0, Math.PI * 2);
            fogCtx.fill();
        });
...
function createConnection(fromId, toId) {
    ...
    const exists = map.connections.some(c => (c.from === fromId && c.to === toId) ||
...
function renderMapMarkersList() {
    ...
    if (countEl)
        countEl.textContent = String(map.markers.length);
    const MARKER_ICONS = window.MARKER_ICONS;
    const MARKER_COLORS = {
...
function filterMapMarkers(query) {
    ...
    const map = getCurrentMap();
    if (!resultsContainer || !map || !map.markers)
        return;
    if (!query.trim()) {
    ...
    const matches = map.markers.filter(m => m.name.toLowerCase().includes(q) ||
...
function initExtendedMapFeatures() {
    try {
        ...
        console.warn('initMapKeyboardShortcuts error:', e);
    }
    try {
        initMapContextMenu();
    }
    catch (e) {
        console.warn('initMapContextMenu error:', e);
    }
    try {
        initMapMeasure();
    }
    catch (e) {
        console.warn('initMapMeasure error:', e);
    }
    try {
        initMapFog();
    }
    catch (e) {
        console.warn('initMapFog error:', e);
    }
    try {
        initMapConnections();
    }
    catch (e) {
        console.warn('initMapConnections error:', e);
    }
    try {
        initCursorTooltip();
    }
    catch (e) {
        console.warn('initCursorTooltip error:', e);
    }
...
// ============================================================
window.addNewMap = addNewMap;
window.renameCurrentMap = renameCurrentMap;
window.deleteCurrentMap = deleteCurrentMap;
window.switchMap = switchMap;
window.renderMapTabs = renderMapTabs;
window.uploadMapToCurrentTab = uploadMapToCurrentTab;
...
window.clearMarkerForm = clearMarkerForm;
window.deleteMarkerFromModal = deleteMarkerFromModal;
window.deleteMarker = deleteMarker;
window.setMapTool = setMapTool;
window.handleQuickPin = handleQuickPin;
window.toggleMapLayer = toggleMapLayer;
window.toggleMapGrid = toggleMapGrid;
window.toggleMapFog = toggleMapFog;
window.toggleMapConnections = toggleMapConnections;
