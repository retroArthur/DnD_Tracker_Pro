...
// DICE ROLLER - ENHANCED - @dice @roll @d20 @damage
// ============================================================
let diceHistory = [];
...
const SKILLS = {
    ...
    dex: [{ name: 'Akrobatik', skill: 'acrobatics' }, { name: 'Fingerfertigkeit', skill: 'sleightOfHand' }, { name: 'Heimlichkeit', skill: 'stealth' }],
    int: [{ name: 'Arkane Kunde', skill: 'arcana' }, { name: 'Geschichte', skill: 'history' }, { name: 'Nachforschungen', skill: 'investigation' }, { name: 'Naturkunde', skill: 'nature' }, { name: 'Religion', skill: 'religion' }],
    wis: [{ name: 'Heilkunde', skill: 'medicine' }, { name: 'Motiv erkennen', skill: 'insight' }, { name: 'Tierumgang', skill: 'animalHandling' }, { name: 'Überleben', skill: 'survival' }, { name: 'Wahrnehmung', skill: 'perception' }],
    cha: [{ name: 'Auftreten', skill: 'performance' }, { name: 'Einschüchtern', skill: 'intimidation' }, { name: 'Täuschung', skill: 'deception' }, { name: 'Überzeugen', skill: 'persuasion' }]
};
...
function rollCustomDice() {
    const notation = $('dice-notation').value.trim();
    if (!notation) return;
    
    const multiCount = parseInt($('dice-multi').value) || 1;
...
function parseDiceNotation(notation) {
    ...
    if (!match) { alert('Ungültige Notation. Beispiele: 2d6+3, 4d6kh3'); return null; }
...
function rollStats() {
    ...
    lastDiceRoll = { notation: 'Stats', total: display, rolls: allRolls.flat() };
    displayDiceResult(display, '📊 Stats (4d6kh3 ×6)', allRolls.flat(), false, false);
    addToDiceHistory('Stats', `Σ${total}`, allRolls.flat());
}
...
function rollCritDamage() {
    ...
    if (result) {
        lastDiceRoll = { notation: `💥 Krit: ${critNotation}`, ...result };
...
function rollAttack(withAdvantage = false) {
    const targetAC = parseInt($('target-ac').value) || 15;
    const bonus = parseInt($('attack-bonus').value) || 0;
    
    let result, notation, rolls;
    if (withAdvantage) {
...
function rollSavingThrow() {
    const dc = parseInt($('save-dc').value) || 15;
    const attr = $('save-attr').value.toUpperCase();
    const mod = parseInt($('save-mod').value) || 0;
...
function rollGroupPerception() {
    ...
    const resultText = results.map(r => `${r.name}: ${r.total} (${r.roll})`).join('\n');
...
function toggleDamageType(chip) {
    ...
    if (!wasSelected) {
        ...
        selectedDamageType = chip.dataset.type;
    } else {
...
function renderDiceHistory() {
    const c = $('dice-history'); if (!c) return;
...
function updateDiceCharStats() {
    const charId = parseInt($('dice-char-select').value);
