...
/**
 * Tests für das Entity-System (Step 7)
 */
...
/**
 * Tests für Error-Handling (Step 8)
 */
...
function runPerformanceTests(silent = false) {
    ...
    if (typeof renderAll === 'function') {
        ...
        renderPending = false; // Reset für Test
    }
...
function testNodeTypes() {
    ...
    const expectedTypes = ['city', 'dungeon', 'forest', 'mountain', 'ruins', 'quest', 'npc', 'player', 'custom'];
...
// Überschreibe save() um Debug-Logging hinzuzufügen
const originalSave = save;
save = function(showMessage) {
    const result = originalSave(showMessage);
    const size = new Blob([JSON.stringify(D)]).size;
    log(`[SAVE] Gespeichert: ${(size/1024).toFixed(1)} KB`);
    return result;
};
