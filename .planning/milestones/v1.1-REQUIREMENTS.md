# Requirements Archive: v1.1 Tech-Debt & Härtung

**Archived:** 2026-07-27
**Status:** SHIPPED
**Audit:** [v1.1-MILESTONE-AUDIT.md](../v1.1-MILESTONE-AUDIT.md) — 11/11 Requirements erfüllt

## Ergebnis der 11 v1.1-Requirements

Alle erfüllt, per 3-Quellen-Abgleich belegt (Phasen-`VERIFICATION.md` + SUMMARY-Frontmatter +
Traceability-Tabelle), keine verwaisten IDs:

| ID | Phase | Ergebnis |
|----|-------|----------|
| TEST-01 | 8 | validiert — 11 Baseline-Fails behoben, davon 3 echte App-Bugs |
| TEST-02 | 8 | validiert — `e2e` blockiert die Deploy-Kette |
| EDIT-01 | 9 | validiert — 0 `execCommand`-Aufrufe in `ui/editors/rich-text.js`, 3 dokumentierte Ausnahmen ausserhalb |
| EDIT-02 | 9 | validiert — Regressionsnetz aus 79 Tests in 4 Dateien |
| EDIT-03 | 9 | validiert — Verhaltensgleichheit gegen die vorab gemessene Baseline |
| SEC-01 | 10 | validiert — Sanitizer-Tests gegen den Produktionsquelltext, Import-Kette abgesichert |
| SEC-02 | 10 | validiert — vier `*-SECURITY.md` (Phasen 1, 2, 9, 10) mit je `threats_open: 0` |
| ARCH-01 | 11 | validiert — eine Liste statt zwei, Hard-Abort bei fehlender Datei |
| ARCH-02 | 11 | validiert — Dedup-Pass 3 entfernt, Quell-Pre-Check davor |
| ARCH-03 | 11 | validiert — CI-Lauf 30216452989: sechs Jobs grün, keine Deprecation-Annotation |
| ARCH-04 | 11 | validiert — Codebase-Map aufgefrischt, jeder CONCERNS-Eintrag disponiert |

---

## ⚠ ÜBERTRAG NACH v1.2 — 27 offene DEBT-Posten

**Diese Liste ist der wichtigste Teil dieses Archivs.** `DEBT-01`..`DEBT-29` entstanden als
vorgesehene dritte Disposition von ARCH-04 („als Requirement übernommen"). Zwei davon wurden noch
in v1.1 geschlossen:

- **DEBT-17** ✅ — Datei-Backup schrieb ab 5 MB Kampagnengröße eine leere Kampagne (`71fb6ef`)
- **DEBT-29** ✅ — die Testlücke, die DEBT-17 verdeckt hatte (`71fb6ef`)

**Die verbleibenden 27 sind offen und müssen beim Aufsetzen von v1.2 übernommen werden.** Die
vollständigen Beschreibungen samt Live-Code-Belegen stehen unten im Abschnitt „v2 Requirements";
die Dispositionen mit Belegen zusätzlich in
[`11-CONCERNS-TRIAGE.md`](v1.1-phases/11-architektur-build-hygiene/11-CONCERNS-TRIAGE.md).

Schwerste offene Posten nach Einschätzung der Triage: **DEBT-18** (Umzugs-Export verliert
IndexedDB-Inhalte — Soundboard-Audio und Würfelstatistik), **DEBT-19** (Audio-Löschen ohne Undo),
**DEBT-21/22** (Datei-Backup sichert nur die aktive Kampagne; Dateinamen können kollidieren).

---

*Der ungekürzte Stand der Datei zum Zeitpunkt des Milestone-Abschlusses folgt.*
...
Ursprung und die Live-Code-Belege in `.planning/milestones/v1.1-phases/11-architektur-build-hygiene/11-CONCERNS-TRIAGE.md`.
...
`.planning/milestones/v1.1-phases/11-architektur-build-hygiene/11-CONCERNS-TRIAGE.md` §„Nach dem Map-Refresh
hinzugekommen".
