...
| `validate.py` | Path(__file__).parent; kein Linux-Hardcode | VERIFIED | Zeile 11: SOURCE_DIR = str(Path(__file__).parent); 0 /mnt/user-data-Treffer |
...
| STAB-09 | 01-05 | Tote Dateien entfernt; python statt python3 | SATISFIED | 7 Dateien geloescht; package.json: kein python3; validate.py: Path(__file__).parent; Hinweis: validate.py schlaegt bei Emoji-Ausgabe mit UnicodeEncodeError auf Windows (cp1252) fehl — kein Pfad-Absturz aber kein sauberer Exit |
