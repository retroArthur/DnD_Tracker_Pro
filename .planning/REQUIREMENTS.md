...
- **DEBT-17** ✅ **BEHOBEN (2026-07-26, `71fb6ef`)**: Datei-Backup schrieb eine leere Kampagne, sobald der IndexedDB-Modus (>5 MB) griff — `systems/spellslots/persistence.js:64-68` loescht den localStorage-Schatten nach dem IDB-Write, `systems/file-backup/file-backup-manager.js` las aber ausschliesslich aus localStorage (`{}` im Fallback); `pruneOldSnapshots()` loeschte danach den aeltesten echten Snapshot. Kumulativer, stiller Totalverlust bei grossen Kampagnen — Ursprung: CONCERNS.md (2026-07-26) §Datenintegrität & Persistenz, triagiert in 11-CONCERNS-TRIAGE.md (Eintrag N1).
  **Fix:** neue `readCampaignDataForBackup()` mit der Reihenfolge localStorage → IndexedDB (`loadFromIndexedDBFallbackRaw`) → laufendes `D`; ein leeres Objekt gilt in keiner Stufe als gueltige Kampagne. Zweite Schutzschicht: liefert keine Quelle Daten, wird gar nichts geschrieben und der Status geht auf `paused` — lieber kein Backup als ein leeres, das die guten Snapshots verdraengt. Abgesichert durch `tests/unit/file-backup-idb.test.js` (7 Tests, TDD: 5 davon zuvor rot).
...
- **DEBT-29** ✅ **BEHOBEN (2026-07-26, `71fb6ef`)**: Kein Test deckte das Zusammenspiel Persistenz-IDB-Modus ↔ Datei-Backup ab — exakt die Testlücke, die DEBT-17 unentdeckt ließ — Ursprung: CONCERNS.md §Test-Lücken, triagiert in 11-CONCERNS-TRIAGE.md (Eintrag N17).
  **Geschlossen durch** `tests/unit/file-backup-idb.test.js`: faehrt `_doBackup()` mit geloeschtem localStorage-Key gegen einen gemockten IDB-Store und deckt zusaetzlich den Fall ab, dass beide Quellen leer sind. Jest damit 621 → 628 Tests, 26 → 27 Suiten.
