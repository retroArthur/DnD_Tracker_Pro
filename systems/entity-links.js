...
function renderInsertLinkTargets() {
    ...
    if (targetType === 'npcs')
        ...
        items = D.locations || [];
    else if (targetType === 'quests')
        items = D.quests || [];
    else if (targetType === 'characters')
        items = D.characters || [];
    else if (targetType === 'encounters')
        items = D.encounters || [];
...
function renderLinkTargets() {
    ...
    if (!targetTypeEl || !select || !sourceTypeEl || !sourceIdEl)
        return;
    const targetType = targetTypeEl.value;
...
function addEntityLink() {
    ...
    if (!sourceTypeEl || !sourceIdEl || !targetTypeEl || !targetIdEl)
        return;
    const sourceType = sourceTypeEl.value;
    const sourceId = parseEntityId(sourceIdEl.value);
    const targetType = targetTypeEl.value;
...
function removeEntityLink(index) {
    ...
    const getEntityByTypeAndId = window.getEntityByTypeAndId;
    const source = getEntityByTypeAndId(sourceType, sourceId);
    if (!source || !source.links || !source.links[index])
        return;
    ...
    if (target && target.links) {
        ...
        if (reverseIdx > -1)
            target.links.splice(reverseIdx, 1);
    }
    renderCurrentLinks();
    const renderAll = window.renderAll;
    const save = window.save;
